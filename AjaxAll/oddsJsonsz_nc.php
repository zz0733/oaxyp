...
if ($tid == 1)
{
	...
	$num_arr = sum_ball_count_1 ($BallString, $BallString_a, $results, 3);
	arsort($num_arr);
	$num_arr = json_encode($num_arr);

	$row_1 = sum_str_s ($results, 0);
	$row_2 = sum_str_s ($results, 1);
	$row_3 = sum_str_s ($results, 2);
	$row_4 = sum_str_s ($results, 3);
	$row_5 = sum_str_s ($results, 4);
	$row_6 = sum_str_s ($results, 5);
	$row_7 = sum_str_s ($results, 6);
	$row_8 = sum_str_s ($results, 7); //1-8號球-號碼
...
else if ($tid == 2)
{
	//獲取封盤時間、開獎時間、刷新時間
	$db = new DB();
	$result = $db->query("SELECT `g_qishu`, `g_feng_date`, `g_open_date` FROM g_kaipan9 WHERE `g_lock` = 2 LIMIT 1 ", 1);
	if ($result && Copyright)
	{
		...
		//取出1-8球和總和龍虎雙面賠率
		$db=new DB();
		$sql = "SELECT  `h1`, `h2`, `h3`, `h4`, `h5`, `h6`, `h7`, `h8`, `h9`, `h10`, `h11`, `h12`, `h13`, `h14`, `h15`, `h16`, `h17`, `h18`, `h19`, `h20`,`g_type` FROM `g_odds9` WHERE `g_type` = 'Ball_1' OR `g_type` = 'Ball_2' OR `g_type` = 'Ball_3' OR `g_type` = 'Ball_4' OR `g_type` = 'Ball_5' OR `g_type` = 'Ball_6' OR `g_type` = 'Ball_7' OR `g_type` = 'Ball_8' ORDER BY g_id ASC ";
		...
		for ($i=0; $i<count($list); $i++){
			...
			foreach ($list[$i] as $key=>$value){
				$arrList[$i][$key] = setodds($key, $value, $ConfigModel, $user, 0,$str);
			}
		}
