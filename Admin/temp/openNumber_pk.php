...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])&&!empty($_POST['s_num2'])&&!empty($_POST['s_num3'])&&
		  ...
		  !empty($_POST['s_num9'])&&!empty($_POST['s_num10'])){
			...
			$carr['g_ball_9'] = $_POST['s_num9'];
			$carr['g_ball_10'] = $_POST['s_num10'];
			...
			if ($db->query("SELECT g_id FROM g_history6 WHERE g_id = '{$_GET['UpNumcid']}' AND g_game_id =6 LIMIT 1", 0)){
				$sql = "UPDATE g_history6 SET 
				...
				$db->query($sql, 2);
				exit(alert_href('更變成功。', 'openNumber_pk.php'));
			}
	} else {
		if (!Matchs::isNumber($_POST['number'],6,6)) exit(back('開獎期數格式錯誤3！'));
		...
		$arr['g_ball_9'] = $_POST['num9'];
		$arr['g_ball_10'] = $_POST['num10'];
		$arr = array_unique($arr);
		//echo count($arr['cry']);
		
		
		//if (count($arr) != 11) exit(back('開獎號碼選擇錯誤！'));
		$sql ="SELECT g_id FROM g_history6 WHERE g_qishu = '{$arr['g_qishu']}' AND g_game_id = 6 AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0)){
			exit(back('第 '.$arr['g_qishu'].' 已經存在！'));
		} else {
			$sql = "INSERT INTO g_history6 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3,g_ball_4,g_ball_5,g_ball_6,g_ball_7,g_ball_8,g_ball_9,g_ball_10) VALUES 
			(
				'{$arr['g_qishu']}',
				'{$arr['g_date']}',
				'6',
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history6 WHERE g_qishu = '{$numberId}' AND g_game_id =6 AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
		$SumAmount = new SumAmountpk($numberId);
...
else if (isset($_GET['numDelid']) || isset($_GET['Numdelid']))
{
	if (isset($_GET['numDelid'])){
		...
		$sql = "DELETE FROM g_history6 WHERE g_date < '{$numDelid}' ";
	} else {
		$numDelid = $_GET['Numdelid'];
		$sql = "DELETE FROM g_history6 WHERE g_id = '{$numDelid}' ";
	}
	$db->query($sql, 2);
	exit(back('刪除成功。'));
}
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history6 WHERE g_id = '{$UpNumcid}' AND g_game_id = 6 LIMIT 1";
...
else 
{
	...
	//取出最近179期未結算注單
	$sql = "SELECT g_qishu FROM g_history6 WHERE g_game_id = 6 ORDER BY g_qishu DESC LIMIT 179";
...
<!--
	...
	function isSumAmount(){
		...
		if (number.value == "" || number.value.length != 6){alert("開獎期數格式錯誤！");return false;}
		if(cry.checked){
                        ...
                        <td background="/Admin/temp/images/tab_05.gif">
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr>
                                    ...
                                    <td><font style="font-weight:bold" color="#344B50">&nbsp;開獎管理--北京赛车PK10</font></td>
                            		<td>
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:30px" align="center">
                                    ...
                                    <?php if (!$UpNumcid){?>
									 ...
									 <td>
                                    <?php if (!$UpNumcid){?>
	                                    <input type="text" name="num9" style="width:50px;" onfocus="this.className='inp1mMM'" onblur="this.className='inp1MM'" class='inp1MM' value="<?php echo $UpNums[0]['g_ball_9']?>" />
	                                    <?php }else {?>
	                                    <input type="text" name="s_num9" style="width:50px;" onfocus="this.className='inp1mMM'" onblur="this.className='inp1MM'" class='inp1MM' value="<?php echo $UpNums[0]['g_ball_9']?>" />
	                                    <?php }?>
	                                </td>
									 <td>
                                    <?php if (!$UpNumcid){?>
	                                    <input type="text" name="num10" style="width:50px;" onfocus="this.className='inp1mMM'" onblur="this.className='inp1MM'" class='inp1MM' value="<?php echo $UpNums[0]['g_ball_10']?>" />
	                                    <?php }else {?>
	                                    <input type="text" name="s_num10"  style="width:50px;" onfocus="this.className='inp1mMM'" onblur="this.className='inp1MM'" class='inp1MM' value="<?php echo $UpNums[0]['g_ball_10']?>" />
	                                    <?php }?>
	                                </td>
									<tr>
									                        <td colspan="13" class="f" align="center">
                            ...
                            <table border="0" cellspacing="0" class="contergd"  id="NumberList" align="center">
                            	<tr class="tr_top">
                                    ...
                                    <td width="260" colspan="10"><b>開出號碼</b></td>
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/Admin/temp/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_pk.php?UpNumcid=<?php echo $numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/Admin/temp/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_pk.php?Numdelid=<?php echo $numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_pk.php?startId=1&numId=<?php echo $NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
