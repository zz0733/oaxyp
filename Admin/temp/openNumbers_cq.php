...
{
	$delid = $_GET['delid'];
	$id = $db->query("SELECT g_lock FROM g_kaipan2 WHERE g_id = '{$delid}' LIMIT 1 ", 0);
	if ($id)
	{
		$db->query("DELETE FROM g_kaipan2 WHERE g_id = '{$delid}' LIMIT 1", 2);
...
{
	$openid = $_GET['openid'];
	$openids = $db->query("SELECT g_lock FROM g_kaipan2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 0);
	if ($openids)
	{
		$db->query("DELETE FROM g_kaipan2 WHERE g_qishu < '{$openid}' ", 2);
		$db->query("UPDATE g_kaipan2 SET g_lock = 2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 2);
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	insertNumbers('09:50:00', $inserid, 10, 24, 143, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan2` ORDER BY g_qishu ASC ", 1);
}
?>
  ...
  <tr>
    ...
    <td class="c"><table border="0" cellspacing="0" class="main">
        <tr>
          ...
          <td background="/Admin/temp/images/tab_05.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><font style="font-weight:bold" color="#344B50">&nbsp;開盤設置--重慶時時彩</font></td>
          ...
          <td class="c"><!-- strat -->
            ...
            <table border="0" cellspacing="0" class="conter">
              ...
              <tr>
                ...
                <td id="showTxt" class="left_p6" style="color:#0000FF;">開盤時間與封盤時間為第一期開始計算、23期封盤后系統默認自動加載明天開盤列表。</td>
              </tr>
              <tr>
                <td height="36" class="bj">總期數</td>
                <td class="left_p6"><input name="number" type="text" id="number" style="width:50px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="120" /></td>
              </tr>
              <tr>
        ...
        <tr>
          ...
          <td class="f" align="center"><span class="odds">（注意：系統會自動加載1-120期盤口）</span> 天:
