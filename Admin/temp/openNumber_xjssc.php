...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])||!empty($_POST['s_num2'])||!empty($_POST['s_num3'])|| !empty($_POST['s_num4'])||!empty($_POST['s_num5']))
	{
		...
		if ($db->query("SELECT g_id FROM g_history10 WHERE g_id = '{$_GET['UpNumcid']}' LIMIT 1", 0))
		{
			$sql = "UPDATE g_history10 SET 
			...
			$db->query($sql, 2);
			exit(alert_href('更變成功。', 'openNumber_xjssc.php'));
		}
	}
	else 
	{
		...
		$sql ="SELECT g_id FROM g_history10 WHERE g_qishu = '{$arr['g_qishu']}' AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0))
		{
		...
		{
			$sql = "INSERT INTO g_history10 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3,g_ball_4,g_ball_5) VALUES 
			(
				'{$_POST['number']}',
				'{$_POST['openDate']}',
				'2',
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history10 WHERE g_qishu = '{$numberId}' AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
...
else if (isset($_GET['numDelid']) || isset($_GET['Numdelid']))
{
	if (isset($_GET['numDelid'])){
		...
		$sql = "DELETE FROM g_history10 WHERE g_date < '{$numDelid}' ";
	} else {
		$numDelid = $_GET['Numdelid'];
		$sql = "DELETE FROM g_history10 WHERE g_id = '{$numDelid}' ";
	}
	$db->query($sql, 2);
	exit(back('刪除成功。'));
}
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history10 WHERE g_id = '{$UpNumcid}' LIMIT 1";
...
else 
{
	...
	//取出最近20期未結算注單
	$sql = "SELECT g_qishu FROM g_history10 ORDER BY g_qishu DESC LIMIT 168";
	$Number = $db->query($sql, 1);
	$NumberArr = array();
	if ($Number){
		for ($i=0; $i<count($Number); $i++){
			$sql = "SELECT g_jiner, g_mingxi_1_str FROM g_zhudan WHERE g_qishu ='{$Number[$i]['g_qishu']}' AND g_type='新疆时时彩' AND g_win is null ";
                        ...
                        <td background="/Admin/temp/images/tab_05.gif">
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr>
                                    ...
                                    <td><font style="font-weight:bold" color="#344B50">&nbsp;開獎管理--新疆時時彩</font></td>
                            		<td>
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/Admin/temp/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_xjssc.php?UpNumcid=<?php echo $numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/Admin/temp/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_xjssc.php?Numdelid=<?php echo $numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_xjssc.php?startId=1&numId=<?php echo $NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
