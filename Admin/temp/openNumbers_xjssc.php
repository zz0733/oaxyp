...
markPos("后台-新疆时时彩盘口管理");
$db=new DB();
$lm='xjssc';
if (isset($_GET['delid']) && Matchs::isNumber($_GET['delid']))
{
	$delid = $_GET['delid'];
	$id = $db->query("SELECT g_lock FROM g_kaipan10 WHERE g_id = '{$delid}' LIMIT 1 ", 0);
	if ($id)
	{
		$db->query("DELETE FROM g_kaipan10 WHERE g_id = '{$delid}' LIMIT 1", 2);
...
{
	$openid = $_GET['openid'];
	$openids = $db->query("SELECT g_lock FROM g_kaipan10 WHERE g_qishu = '{$openid}' LIMIT 1 ", 0);
	if ($openids)
	{
		$db->query("DELETE FROM g_kaipan10 WHERE g_qishu < '{$openid}' ", 2);
		$db->query("UPDATE g_kaipan10 SET g_lock = 2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 2);
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	InsertNumber_xjssc($inserid, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan10` ORDER BY g_qishu ASC ", 1);
}
?>
...
<!--
	function delNumber(id, sInt){
		...
		if (sInt == 1){
			...
			if (confirm("警告：系統將會自動重新加載1-120期，開獎、封盤時間。\n你確定嗎？")){
				href = "?inserid=";
  ...
  <tr>
    ...
    <td class="c"><table border="0" cellspacing="0" class="main">
        <tr>
          ...
          <td background="/Admin/temp/images/tab_05.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><font style="font-weight:bold" color="#344B50">&nbsp;開盤設置--新疆時時彩</font></td>
          ...
          <td class="c"><!-- strat -->
            ...
            <table border="0" cellspacing="0" class="conter">
              ...
              <tr>
                ...
                <td id="showTxt" class="left_p6" style="color:#0000FF;">開盤時間與封盤時間為第一期開始計算、96期封盤后系統默認自動加載明天開盤列表。</td>
              </tr>
              <tr>
                <td height="36" class="bj">總期數</td>
                <td class="left_p6"><input name="number" type="text" id="number" style="width:50px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="96" /></td>
              </tr>
              <tr>
        ...
        <tr>
          ...
          <td class="f" align="center"><span class="odds">（注意：系統會自動加載1-96期盤口）</span> 天:
