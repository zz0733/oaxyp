...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if ($ConfigModel['g_news_lock'] == 1 && $cid == 0){
		...
		'{$news['RankShow']}')";
		$db->query($sql, 2);
		exit(alert_href('新增公告成功。', 'newsInfo.php'));
	} else {
		...
		if ($db->query("SELECT g_text FROM g_news WHERE g_id = '{$cid}' LIMIT 1", 0)){
			...
			exit(alert_href('更變公告成功。', 'newsInfo.php'));
		}
	}
}
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
								...
								<td class="bj1" align="right">公告內容
                                	...
                                	<td class="left_p61"><textarea rows="6" width="100%" height="50%" cols="40" id="Editors" name="Editors" ><?php echo $Editors?></textarea></td>
                                </tr>	
                                <tr>
								<td class="bj1" align="right">選項
								</td>
                    				<td align="left">&nbsp; &nbsp; 
                    					會員頁:<input style="position:relative;top:2px" name="NumberShow" <?php if($NumberShow ==1){echo' checked="checked"';}?> type="checkbox"/>&nbsp; &nbsp; &nbsp; &nbsp; 
                    					提示窗:<input style="position:relative;top:2px" <?php if($NumberAlertShow ==1){echo' checked="checked"';}?> name="NumberAlertShow" type="checkbox" />&nbsp; &nbsp; &nbsp; &nbsp; 
                    					代理頁:<input style="position:relative;top:2px" <?php if($RankShow ==1){echo' checked="checked"';}?> name="RankShow" type="checkbox" />
                    				</td>
                    			</tr>
                            </table>	
                        <!-- end -->
