...
{
	$delid = $_GET['delid'];
	$id = $db->query("SELECT g_lock FROM g_kaipan8 WHERE g_id = '{$delid}' LIMIT 1 ", 0);
	if ($id)
	{
		$db->query("DELETE FROM g_kaipan8 WHERE g_id = '{$delid}' LIMIT 1", 2);
		exit(alert_href('刪除成功', 'openNumbers_kl8.php'));
	}
	else 
	{
...
{
	$openid = $_GET['openid'];
	$openids = $db->query("SELECT g_lock FROM g_kaipan8 WHERE g_qishu = '{$openid}' LIMIT 1 ", 0);
	if ($openids)
	{
		$db->query("DELETE FROM g_kaipan8 WHERE g_qishu < '{$openid}' ", 2);
		$db->query("UPDATE g_kaipan8 SET g_lock = 2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 2);
		exit(alert_href('操作成功', 'openNumbers_kl8.php'));
	}
} 
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	$inserid = $_GET['inserid'];
	InsertNumber_kl8($inserid, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'openNumbers_kl8.php'));
}
$result=mysql_query("select * from g_kaipan8  order by g_qishu desc LIMIT 1");
        ...
        <tr>
          ...
          <td background="/Admin/temp/images/tab_05.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><font style="font-weight:bold" color="#344B50">&nbsp;開盤設置--快樂8</font></td>
        ...
        <tr>
          ...
          <td class="c"><table border="0" cellspacing="0" class="conter">
              ...
              <tr>
                ...
                <td class="left_p6"><input name="stratTime" type="text" id="stratTime" style="width:110px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="09-29 09:00:00" /></td>
              </tr>
              <tr>
                <td height="36" class="bj">開獎時間</td>
                <td class="left_p6"><input name="endTime" type="text" id="endTime" style="width:110px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="09-29 09:05:00" /></td>
              </tr>
            ...
            <table border="0" cellspacing="0" class="conter">
              ...
              <?php	
		 $result = mysql_query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan8` ORDER BY g_qishu ASC "); 
