...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])&&!empty($_POST['s_num2'])&&!empty($_POST['s_num3'])&&
		  ...
		  !empty($_POST['s_num7'])&&!empty($_POST['s_num8'])){
			...
			if ($db->query("SELECT g_id FROM g_history9 WHERE g_id = '{$_GET['UpNumcid']}' LIMIT 1", 0)){
				$sql = "UPDATE g_history9 SET 
				...
				$db->query($sql, 2);
				exit(alert_href('更變成功。', 'openNumber_nc.php'));
			}
	} else {
		...
		$sql ="SELECT g_id FROM g_history9 WHERE g_qishu = '{$arr['g_qishu']}' AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0)){
			exit(back('第 '.$arr['g_qishu'].' 已經存在！'));
		} else {
			$sql = "INSERT INTO g_history9 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3,g_ball_4,g_ball_5,g_ball_6,g_ball_7,g_ball_8) VALUES 
			(
				'{$arr['g_qishu']}',
				'{$arr['g_date']}',
				'1',
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history9 WHERE g_qishu = '{$numberId}' AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
...
else if (isset($_GET['numDelid']) || isset($_GET['Numdelid']))
{
	if (isset($_GET['numDelid'])){
		...
		$sql = "DELETE FROM g_history9 WHERE g_date < '{$numDelid}' ";
	} else {
		$numDelid = $_GET['Numdelid'];
		$sql = "DELETE FROM g_history9 WHERE g_id = '{$numDelid}' ";
	}
	$db->query($sql, 2);
	exit(back('刪除成功。'));
}
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history9 WHERE g_id = '{$UpNumcid}' LIMIT 1";
...
else 
{
	...
	//取出最近84期未結算注單
	$sql = "SELECT g_qishu FROM g_history9 ORDER BY g_qishu DESC LIMIT 84";
...
<!--
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/AjaxAll/numberUp.php", {mid : 1, num : num, type : 9}, function(data){
				if(data != 1){
                        ...
                        <td background="/Admin/temp/images/tab_05.gif">
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr>
                                    ...
                                    <td><font style="font-weight:bold" color="#344B50">&nbsp;開獎管理--幸运农场</font></td>
                            		<td>
                        ...
                        <!-- strat -->
                            ...
                            <table border="0" cellspacing="0" class="contergd"  id="NumberList" align="center">
                            	<tr class="tr_top">
                                    ...
                                    <td width="140">開獎時間</td>
                                    <td width="308" colspan="8"><b>開出號碼</b></td>
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/Admin/temp/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_nc.php?UpNumcid=<?php echo $numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/Admin/temp/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_nc.php?Numdelid=<?php echo $numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_nc.php?startId=1&numId=<?php echo $NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
