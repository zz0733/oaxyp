...
markPos("后台-极速时时彩盘口管理");
$db=new DB();
$lm='jxssc';
if (isset($_GET['delid']) && Matchs::isNumber($_GET['delid']))
{
	$delid = $_GET['delid'];
	$id = $db->query("SELECT g_lock FROM g_kaipan3 WHERE g_id = '{$delid}' LIMIT 1 ", 0);
	if ($id)
	{
		$db->query("DELETE FROM g_kaipan3 WHERE g_id = '{$delid}' LIMIT 1", 2);
...
{
	$openid = $_GET['openid'];
	$openids = $db->query("SELECT g_lock FROM g_kaipan3 WHERE g_qishu = '{$openid}' LIMIT 1 ", 0);
	if ($openids)
	{
		$db->query("DELETE FROM g_kaipan3 WHERE g_qishu < '{$openid}' ", 2);
		$db->query("UPDATE g_kaipan3 SET g_lock = 2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 2);
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	InsertNumber_jxssc($inserid, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan3` ORDER BY g_qishu ASC ", 1);
}
?>
        ...
        <tr>
          ...
          <td background="/Admin/temp/images/tab_05.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><font style="font-weight:bold" color="#344B50">&nbsp;開盤設置--极速时时彩</font></td>
