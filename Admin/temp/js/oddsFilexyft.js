...
var Urls = "/Admin/temp/ajaxad/jsonxyft.php";
var EstateTime = 90;
var EndTime = 90;
var StartTime = 90;
...
function setList () {
	$.post (Urls, {typeid : 3}, function (data) {
		if (data.num != ""){
			...
			var cHtml = '<tr class="tr_top"><th colspan="2">兩面長龍</th></tr>';
...
function setOdds (oddsList){
	for (var k =0;k<oddsList.length;k++){
		
	for (var i in oddsList[k]){
		//var _odds = isFloat(oddsList[k][i]);
		var _odds = oddsList[k][i];
		switch(k){
		case 0 :$("#t11_"+i).html(_odds);$("#t3_"+i).html(_odds);$("#t7_"+i).html(_odds);break;
		case 1 :$("#t12_"+i).html(_odds);$("#t4_"+i).html(_odds);$("#t8_"+i).html(_odds);break;
		case 2 :$("#t1_"+i).html(_odds);$("#t5_"+i).html(_odds);$("#t9_"+i).html(_odds);break;
		case 3 :$("#t2_"+i).html(_odds);$("#t6_"+i).html(_odds);$("#t10_"+i).html(_odds);break;
		}
		var w = i.substr(1);
		$("#w"+w).html(oddsList[k][i]);
	}
	}
}
...
function sumIsNumLose (infoList, cid){
	if (infoList.list != "" && infoList.list_s !="" && infoList.list_s!=null && infoList.list != null ){
...
function GoPos($this, sInt){
	if (EndTime > 1) {
		...
		if (sInt == 1) {
			...
			s1=s.split('_');
			switch(s1[0]){
				case 't1' : $("#s_type1").html('冠军');break;
				case 't2' : $("#s_type1").html('亚军');break;
				case 't3' : $("#s_type1").html('第三名');break;
				case 't4' : $("#s_type1").html('第四名');break;
				case 't5' : $("#s_type1").html('第五名');break;
				case 't6' : $("#s_type1").html('第六名');break;
				case 't7' : $("#s_type1").html('第七名');break;
				case 't8' : $("#s_type1").html('第八名');break;
				case 't9' : $("#s_type1").html('第九名');break;
				case 't10' : $("#s_type1").html('第十名');break;
				case 't11' : $("#s_type1").html('冠、亞軍和');break;
				case 't12' : $("#s_type1").html('冠亞和');break;
			}
		 	var h = $("#"+s1[0]+"_h"+s1[1]).html();
		 	var n = $("#n"+s).html();
		 	var a = $("#a"+s).html();
		 	var b = $("#b"+s).html();
		 	if (a != "-" && a > 0) {
...
function GoPost (sInt){
	if (sInt == 1){
		...
		var s_type = $("#s_type1").html();
		
		switch(s_type){
		case '冠亞和' : var s_num = s_type + $("#type_s").text();break;
		default: var s_num = $("#type_s").text();break;
		}
		//var s_num = $("#type_s").text();
