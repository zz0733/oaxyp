var url = "/Admin/temp/ajaxad/oddsJsonxjssc.php";
...
function setOdds(othe,ty){
	$.post(url,{mid:6},function(data){
		...
		var a=othe,odds_1=0.01,odds_2=100;
		if(jsright(othe,2)>10){odds_1=0.001;odds_2=1000;}
		if(jsleft(othe,1)=="g"){if(jsright(othe,1)<7){odds_1=0.001;odds_2=1000;}}
		if(jsleft(othe,1)=="i"||jsleft(othe,1)=="s"||jsleft(othe,1)=="x"){odds_1=0.1;odds_2=10;}
		$.post(url,{mid:7,odds:othe,ty:ty},function(date){
//			alert(date);
			if(date=="true")
				if(ty==1)
					document.getElementById(a).innerHTML=Math.round((parseFloat(document.getElementById(a).innerHTML)+odds_1)*odds_2)/odds_2;
				else
					document.getElementById(a).innerHTML=Math.round((parseFloat(document.getElementById(a).innerHTML)-odds_1)*odds_2)/odds_2;
				}
		);
...
function loadInfo(){//暂时取消。访问频繁 Add By 夜貓@2013.8.18
...
function GoPos($this, s, sint){
	...
	//alert("#"+_s+sint.substring(2, sint.length));
