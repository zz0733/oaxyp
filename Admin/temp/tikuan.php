...
if ($dmoney>$result[0]['g_money']) {
$sql="update g_user set g_money=0,g_money_yes=g_money_yes-{$dmoney} where g_name='{$_POST['uname']}'";
}else{
$sql="update g_user set g_money=g_money-{$dmoney},g_money_yes=g_money_yes-{$dmoney} where g_name='{$_POST['uname']}'";
}
$db->query($sql, 2);

if ($dmoney>$result[0]['g_money_yes']) {
$sql="INSERT INTO g_qdetail SET  g_state='提取', g_name='".$result[0]['g_name']."',g_Money='{$dmoney}',g_autoc='0',g_date='".date("Y-m-d H:i:s")."' ";
}else{
$sql="INSERT INTO g_qdetail SET  g_state='提取', g_name='".$result[0]['g_name']."',g_Money='{$dmoney}',g_autoc='{$amoney}',g_date='".date("Y-m-d H:i:s")."' ";
}
$db->query($sql, 2);

$dmoney>0? $state='已提取':$state='扣除金额';
exit(back('提取完成！'));
}
