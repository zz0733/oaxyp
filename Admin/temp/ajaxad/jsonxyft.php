...
JSON;
	...
	{
		//雙面長龍
		global $BallStringxyft, $BallString_axyft;
		$result = history_resultpk(0,4);
		$num_arr = sum_ball_count_1_pk ($BallStringpk, $BallString_apk, $result, 2);
		arsort($num_arr);
...
JSON;
	...
	{
		$cid = $_POST['cid'];
		$userReportInfo = new UserReportInfoxyft($Users, $cid);
				...
				if ($Users[0]['g_login_id'] ==89 || $Users[0]['g_login_id'] ==56){
					...
					if ($dayMoney < 0 || $_POST['s_money'] > $dayMoney){
						...
						if ($_POST['s_odds'] == 'LM')
						{
							...
							$odds = $db->query("SELECT g_odds4 FROM g_zhudan WHERE g_qishu='{$arr['s_number']}' AND  
						...
						if ($error == null){
							$arr['g_typeid'] = isset($_POST['cid']) ? '重慶時時彩' : '极速赛车';
							$userReportInfo = new UserReportInfoxyft($Users, 0);
...
JSON;
	...
	{
		$opNumber = $db->query("SELECT `g_qishu` FROM `g_history4` WHERE g_ball_1 is not null ORDER BY g_qishu DESC LIMIT 1 ", 0);
		echo $opNumber[0][0];
	}
	else if ($typeID == 8)
	{
		...
		$H = $_POST['hid'];
		$odds = $_POST['oid'];
		$sql = "UPDATE g_odds4 SET `{$H}` = '{$odds}' WHERE g_type = '{$Ball}' ";
		$db->query($sql, 2);
		echo 1;
	}
	else if ($typeID == 9)
	...
	else if ($typeID == 'kaijiang')
	{
		//最新开奖记录
		$db = new DB();
		$result = $db->query("SELECT `g_qishu`, `g_ball_1`, `g_ball_2`, `g_ball_3`, `g_ball_4`, `g_ball_5`, `g_ball_6`, `g_ball_7`, `g_ball_8`, `g_ball_9`, `g_ball_10` FROM `g_history4` WHERE g_ball_1 is not null ORDER BY g_qishu DESC LIMIT 1 ", 0);
		$number = $result[0][0];
		$ballArr = array();
