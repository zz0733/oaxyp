...
//	dump($Users);
	...
	if ($mid == 1)
	{
		include_once ROOT_PATH.'classed/UserReporInfocq.php';
...
JSON;
	...
	if ($mid == 2)
	{
		$sql = "SELECT `h1`, `h2`, `h3`, `h4`, `h5`, `h6`, `h7`, `h8`, `h9`, `h10`, `h11`, `h12`, `h13`, `h14` FROM `g_odds11`  ORDER BY g_id ASC";
...
JSON;
	...
	if ($mid == 3)
	{
		$sql = "SELECT g_qishu FROM g_history11 WHERE g_ball_1 is not null AND g_ball_2 is not null ORDER BY g_id DESC LIMIT 1";
	...
	if ($mid == 7){
		...
		if(!in_array($p[0],array("a","b","c","d","e","g","i","s","x"))||!preg_match("/[0-9]{0,2}/",$p[1])){
			...
			if($_SESSION["loginId"]==89){
				if($_POST["ty"]==1)
					//$sql="update g_odds11 set h".$p[1]."=round(h".$p[1]."+0.01,2) where g_type='{$ball}'";
					$sql="update g_odds11 set h".$p[1]."=round(h".$p[1]."+{$odds_1},{$odds_2}) where g_type='{$ball}'";
				else 
					$sql="update g_odds11 set h".$p[1]."=round(h".$p[1]."-{$odds_1},{$odds_2}) where g_type='{$ball}'";
				if($db->query($sql,2)){
					echo "true";
