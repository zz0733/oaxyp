			...
			if ($endTime < 0){
				...
				if ($Users[0]['g_login_id'] ==89 || $Users[0]['g_login_id'] ==56){
					...
					if ($dayMoney < 0 || $_POST['s_money'] > $dayMoney){
						...
						if($_POST['s_type']=='長牌')
						{
							$arr['s_num'] = array(substr($_POST['s_num'],0,1).','.substr($_POST['s_num'],1,1));	
						}
						elseif($_POST['s_type']=='短牌')
						{
							$arr['s_num'] = array(substr($_POST['s_num'],0,1));	
						}
						else
						...
						if ($_POST['s_odds'] == 'LM')
						{
							...
							$odds = $db->query("SELECT g_odds9 FROM g_zhudan WHERE g_qishu='{$arr['s_number']}' AND  
...
JSON;
	...
	{
		$opNumber = $db->query("SELECT `g_qishu` FROM `g_history9` WHERE g_ball_1 is not null ORDER BY g_qishu DESC LIMIT 1 ", 0);
		echo $opNumber[0][0];
	}
	else if ($typeID == 8)
	{
		...
		$H = $_POST['hid'];
		$odds = $_POST['oid'];
		$sql = "UPDATE g_odds9 SET `{$H}` = '{$odds}' WHERE g_type = '{$Ball}' ";
		$db->query($sql, 2);
		echo 1;
	}
	else if ($typeID == 9)
