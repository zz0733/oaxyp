...
{
	if (!empty($_POST['s_num1'])||!empty($_POST['s_num2'])||!empty($_POST['s_num3'])|| !empty($_POST['s_num4'])||!empty($_POST['s_num5']))
	{
		...
		if ($db->query("SELECT g_id FROM g_history2 WHERE g_id = '{$_GET['UpNumcid']}' AND g_game_id =2 LIMIT 1", 0))
		{
			$sql = "UPDATE g_history2 SET 
			...
			$db->query($sql, 2);
			exit(alert_href('更變成功。', 'openNumber_cq.php'));
		}
	}
	else 
	{
		if (!Matchs::isNumber($_POST['number'],11,11)) exit(back('開獎期數格式錯誤！'));
		for ($i=1; $i<=5; $i++)
		{
		...
		$sql ="SELECT g_id FROM g_history2 WHERE g_qishu = '{$arr['g_qishu']}' AND g_game_id = 2 AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0))
		{
		...
		{
			$sql = "INSERT INTO g_history2 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3,g_ball_4,g_ball_5) VALUES 
			(
				'{$_POST['number']}',
				'{$_POST['openDate']}',
				'2',
			...
			if ($db->query($sql, 2))
			{
				...
				{
					$SumAmountcq = new SumAmountcq($arr['g_qishu']);
					$SumAmountcq->ResultAmount();
				}
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history2 WHERE g_qishu = '{$numberId}' AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
		$Amount = new SumAmountcq($numberId);
...
else if (isset($_GET['numDelid']) || isset($_GET['Numdelid']))
{
	if (isset($_GET['numDelid'])){
		...
		$sql = "DELETE FROM g_history2 WHERE g_date < '{$numDelid}' ";
	} else {
		$numDelid = $_GET['Numdelid'];
		$sql = "DELETE FROM g_history2 WHERE g_id = '{$numDelid}' ";
	}
	$db->query($sql, 2);
	exit(back('刪除成功。'));
}
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history2 WHERE g_id = '{$UpNumcid}' AND g_game_id = 2 LIMIT 1";
...
else 
{
	...
	//取出最近20期未結算注單
	$sql = "SELECT g_qishu FROM g_history2 WHERE g_game_id = 2 ORDER BY g_qishu DESC LIMIT 168";
	$Number = $db->query($sql, 1);
	$NumberArr = array();
	if ($Number){
		for ($i=0; $i<count($Number); $i++){
			$sql = "SELECT g_jiner, g_mingxi_1_str FROM g_zhudan WHERE g_qishu ='{$Number[$i]['g_qishu']}' AND g_type='重慶時時彩' AND g_win is null ";
...
<!--
	...
	function isSumAmount(){
		...
		if (number.value == "" || number.value.length != 11){alert("開獎期數格式錯誤！");return false;}
		if(cry.checked){
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/AjaxAll/numberUp.php", {mid : 1, num : num, type : 2}, function(data){
				if(data != 1){
            	...
            	<table border="0" cellspacing="0" class="main">
                        ...
                        <td background="/Admin/temp/images/tab_05.gif">
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr>
                                    ...
                                    <td><font style="font-weight:bold" color="#344B50">&nbsp;開獎管理--重慶時時彩</font></td>
                            		<td>
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:30px" align="center">
                                	...
                                	<td>
                                	<?php if (!$UpNumcid){?>
                                	<input type="text" name="number" id="number" style="width:100px;text-align:center" onfocus="this.className='inp1mMM'" onblur="this.className='inp1MM'" class='inp1MM' />
                                	<?php } else { echo $UpNums[0]['g_qishu'];}?>
                                	</td>
                                	<td><input style="width:130px;text-align:center" onfocus="this.className='inp1mMM'" onblur="this.className='inp1MM'" class='inp1MM' id="openDate" name="openDate" value='<?php if(!$UpNumcid){echo date('Y-m-d H:i:s');}else{echo $UpNums[0]['g_date'];} ?>' 
                                	/></td>
                                    <td>
                                    <?php if (!$UpNumcid){?>
                    ...
                    <tr>
                        <td colspan="11" class="f" align="center">
                            ...
                            <table border="0" cellspacing="0" class="contercq"  id="NumberList" align="center">
                            	<tr class="tr_top">
                            		<td width="120">開獎期數</td>
                                    <td width="130">開獎時間</td>
                                    <td width="135" colspan="5"><b>開出號碼</b></td>
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/Admin/temp/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_cq.php?UpNumcid=<?php echo $numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/Admin/temp/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_cq.php?Numdelid=<?php echo $numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_cq.php?startId=1&numId=<?php echo $NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
