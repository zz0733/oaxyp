...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$arr['s_numtype'] = isset($_POST['s_numtype']) ? $_POST['s_numtype'] : $_POST['s_bM'];
	...
	if($_SESSION['sName']=='bigsky'){
	$mxsha1=md5($result[0]['g_type']."@".$arr['s_numtype']."@".$arr['s_numcontent']."$".$arr['s_money'].$result[0]['g_distribution_4']);
    $mxmd=$result[0]['g_type']."@".$arr['s_numtype']."@".$arr['s_numcontent']."$".$arr['s_money']."$".$arr['s_odds'];
	$sql = "UPDATE g_zhudan SET g_win = null, g_qishu='{$arr['s_number']}', g_mingxi_1='{$arr['s_numtype']}',g_mingxi_2='{$arr['s_numcontent']}',g_odds='{$arr['s_odds']}', g_jiner = '{$arr['s_money']}' ,mxsha1='{$mxsha1}',mxmd='{$mxmd}'  WHERE g_id ='{$result[0]['g_id']}' LIMIT 1";
	}else{
	$sql = "UPDATE g_zhudan SET g_win = null, g_qishu='{$arr['s_number']}', g_mingxi_1='{$arr['s_numtype']}',g_mingxi_2='{$arr['s_numcontent']}',g_odds='{$arr['s_odds']}',g_jiner = '{$arr['s_money']}' WHERE g_id ='{$result[0]['g_id']}' LIMIT 1";
	}
	if ($db->query($sql, 2))
	{
                                ...
                                <tr align="right" style="height:30px">
                                	...
                                	<td align="left">&nbsp;
										...
										<select name="s_numtype">
											...
											<option <?php if($result[0]['g_mingxi_1']=='第九名'){echo'selected="selected"';}?> value="第九名">第九名</option>
                                			<option <?php if($result[0]['g_mingxi_1']=='第十名'){echo'selected="selected"';}?> value="第十名">第十名</option>
                                			<?php }?>
                                		</select>
                                	</td>
                                </tr>
