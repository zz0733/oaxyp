...
{
	$delid = $_GET['delid'];
	$id = $db->query("SELECT g_lock FROM g_kaipan9 WHERE g_id = '{$delid}' LIMIT 1 ", 0);
	if ($id)
	{
		$db->query("DELETE FROM g_kaipan9 WHERE g_id = '{$delid}' LIMIT 1", 2);
		exit(alert_href('刪除成功', 'openNumbers_nc.php'));
	}
	else 
	{
...
if (isset($_GET['openid']) && Matchs::isNumber($_GET['openid'], 10, 10))
{
	$openid = $_GET['openid'];
	$openids = $db->query("SELECT g_lock FROM g_kaipan9 WHERE g_qishu = '{$openid}' LIMIT 1 ", 0);
	if ($openids)
	{
		$db->query("DELETE FROM g_kaipan9 WHERE g_qishu < '{$openid}' ", 2);
		$db->query("UPDATE g_kaipan9 SET g_lock = 2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 2);
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	InsertNumber_nc('09:53:00', $inserid, 10, 14, 110, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan9` ORDER BY g_qishu ASC ", 1);
}
?>
...
<!--
	function delNumber(id, sInt){
		...
		if (sInt == 1){
			...
			if (confirm("警告：系統將會自動重新加載1-97期，開獎、封盤時間。\n你確定嗎？")){
				href = "?inserid=";
  ...
  <tr>
    ...
    <td class="c"><table border="0" cellspacing="0" class="main">
        <tr>
          ...
          <td background="/Admin/temp/images/tab_05.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><font style="font-weight:bold" color="#344B50">&nbsp;開盤設置--幸运农场</font></td>
          ...
          <td class="c"><!-- strat -->
            ...
            <table border="0" cellspacing="0" class="conter">
              ...
              <tr>
                ...
                <td  class="left_p6" style="color:#0000FF;">最多新增一天97期數，系統將自動讀取開盤時間。</td>
              </tr>
              <tr>
                <td height="36" class="bj">總期數</td>
                <td class="left_p6"><input name="number" type="text" id="number" style="width:50px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="97" /></td>
              </tr>
              <tr>
                <td height="36" class="bj">封盤時間</td>
                <td class="left_p6"><input name="stratTime" type="text" id="stratTime" style="width:110px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="09-19 09:08:30" /></td>
              </tr>
              <tr>
                <td height="36" class="bj">開獎時間</td>
                <td class="left_p6"><input name="endTime" type="text" id="endTime" style="width:110px;" onfocus="this.className='inp1m'" onblur="this.className='inp1'" class="inp1" value="09-19 09:10:00" /></td>
              </tr>
              <tr align="center" style="background:#F6FAFF; height:28px;">
                <td colspan="2"><input type="submit" class="inputs" onclick="delNumber(1, 3)" value="加載盤口" <?php echo $disabled?> /></td>
              </tr>
            </table>
            <table border="0" cellspacing="0" class="conter">
              <tr class="tr_top">
        ...
        <tr>
          ...
          <td class="f" align="center"><span class="odds">（注意：系統會自動加載1-97期盤口）</span> 天:
