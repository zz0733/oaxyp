...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	{
//		dump($sql);
		$SumAmount = new SumAmountkl8($arr['s_number'], false, $arr['uid'], $sum);
...
{
	$sql = "SELECT g_id FROM g_zhudan WHERE g_t_id = '{$uid}' AND g_type = '快樂8' AND g_qishu = '{$result[0]['g_qishu']}}' LIMIT 1 ";
	if ($db->query($sql, 0))
		exit(back('很遺憾！'.$uid.'# 注單已有關聯走飛記錄，無法修改。'));
	
	$ps = false;
	if (!$db->query("SELECT g_qishu FROM g_history8 WHERE g_qishu = '{$result[0]['g_qishu']}' LIMIT 1 ", 0)){
		if ($db->query("SELECT g_id FROM g_kaipan8 WHERE g_qishu = '{$result[0]['g_qishu']}' LIMIT 1", 0))
			$ps = true;
		else
                                ...
                                <tr align="right" style="height:30px">
                                	...
                                	<td align="left">&nbsp;
                                		<select name="s_numcontent">
                                			<?php 
											switch($result[0]['g_mingxi_1'])
											{
												case "正碼":
													for($i=1;$i<=80;$i++)
													{
														$k=sprintf('%02d',$i);
														echo "<option value='{$k}' ".($k==$result[0]['g_mingxi_2']?"selected":"").">{$k}</option>\r\n";	
													}
													break;	
												case "總和大小":
													echo "<option value='總和大' ".('總和大'==$result[0]['g_mingxi_2']?"selected":"").">總和大</option>\r\n";	
													echo "<option value='總和小' ".('總和小'==$result[0]['g_mingxi_2']?"selected":"").">總和小</option>\r\n";	
													break;
												case "總和單雙":
													echo "<option value='總和單' ".('總和單'==$result[0]['g_mingxi_2']?"selected":"").">總和單</option>\r\n";	
													echo "<option value='總和雙' ".('總和雙'==$result[0]['g_mingxi_2']?"selected":"").">總和雙</option>\r\n";
													break;
												case "總和和局":
													echo "<option value='總和810' ".('總和810'==$result[0]['g_mingxi_2']?"selected":"").">總和810</option>\r\n";
													break;
												case "總和過關":
													echo "<option value='總大單' ".('總大單'==$result[0]['g_mingxi_2']?"selected":"").">總大單</option>\r\n";	
													echo "<option value='總大雙' ".('總大雙'==$result[0]['g_mingxi_2']?"selected":"").">總大雙</option>\r\n";
													echo "<option value='總小單' ".('總小單'==$result[0]['g_mingxi_2']?"selected":"").">總小單</option>\r\n";	
													echo "<option value='總小雙' ".('總小雙'==$result[0]['g_mingxi_2']?"selected":"").">總小雙</option>\r\n";	
													break;
												case "前後和":
													echo "<option value='前(多)' ".('前(多)'==$result[0]['g_mingxi_2']?"selected":"").">前(多)</option>\r\n";	
													echo "<option value='後(多)' ".('後(多)'==$result[0]['g_mingxi_2']?"selected":"").">後(多)</option>\r\n";
													echo "<option value='前後(和)' ".('前後(和)'==$result[0]['g_mingxi_2']?"selected":"").">前後(和)</option>\r\n";	
													break;
												case "單雙和":
													echo "<option value='單(多)' ".('單(多)'==$result[0]['g_mingxi_2']?"selected":"").">單(多)</option>\r\n";	
													echo "<option value='雙(多)' ".('雙(多)'==$result[0]['g_mingxi_2']?"selected":"").">雙(多)</option>\r\n";
													echo "<option value='單雙(和)' ".('單雙(和)'==$result[0]['g_mingxi_2']?"selected":"").">單雙(和)</option>\r\n";	
													break;
												case "五行":
													echo "<option value='金' ".('金'==$result[0]['g_mingxi_2']?"selected":"").">金</option>\r\n";	
													echo "<option value='木' ".('木'==$result[0]['g_mingxi_2']?"selected":"").">木</option>\r\n";
													echo "<option value='水' ".('水'==$result[0]['g_mingxi_2']?"selected":"").">水</option>\r\n";	
													echo "<option value='火' ".('火'==$result[0]['g_mingxi_2']?"selected":"").">火</option>\r\n";
													echo "<option value='土' ".('土'==$result[0]['g_mingxi_2']?"selected":"").">土</option>\r\n";	
													break;
											}
                                			?>
                                		</select>
