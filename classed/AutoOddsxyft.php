...
class AutomaticOddsxyft 
{
	...
	public function UpExecution()
	{
		...
		if ($result['Num'] && Copyright)
		{
			foreach ($result['Num'] as $key=>$value)
			{
				if($key!="")
				{
					$sql = "UPDATE g_odds4 SET `{$key}`={$key}-{$value} WHERE g_type <> 'Ball_11' AND g_type <> 'Ball_12'";
					$this->db->query($sql, 2);
				}
			}
		}
		if ($result['Str'] && Copyright)
		{
			for ($i=0; $i<count($result['Str']); $i++)
			{
				if($result['Str'][$i][1]!="")
				{
					$sql = "UPDATE g_odds4 SET `{$result['Str'][$i][1]}` = {$result['Str'][$i][1]}-{$result['Str'][$i][2]} WHERE g_type ='{$result['Str'][$i][0]}' LIMIT 1 ";
					$this->db->query($sql, 2);
				}
			}
		}
	...
	{
		$ResultNumber = history_resultxyft(0,4);
		
		//得到雙面無出期數
		global $BallStringxyft,$BallString_axyft;
		$NumberStrArr = sum_ball_count_1_xyft ($BallStringxyft, $BallString_axyft, $ResultNumber, 1);
		...
		//得到1-20無出期數
		$NumberArr = sum_ball_count_xyft ($ResultNumber, 1);
