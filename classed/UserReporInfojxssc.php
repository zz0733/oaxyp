...
class UserReportInfojxssc 
{
	...
	{
		$sql = "SELECT  `g_qishu`, `g_feng_date`, `g_open_date` FROM `g_kaipan3` WHERE g_lock = 2 ORDER BY g_qishu ASC LIMIT 1";
	...
	public function GetInfojxssc()
	{
		$sql = "SELECT g_qishu FROM g_kaipan3 WHERE g_lock = 2 ORDER BY g_qishu DESC LIMIT 1";
...
//		dump($number);
	...
	private function SumReport($result, $logId)
	{
		...
		else if ($logId == 56)
		{
			...
			if ($a == 0){
				if($result['g_nid']=='分公司走飛')
				{
					$List['g_jiner'] =$result['g_jiner'];		
				}
				else
				{
					$List['g_jiner'] =0;//$result['g_jiner'];
					$List['g_tueishui']=0;
				}
			}else {
