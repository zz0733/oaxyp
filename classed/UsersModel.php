...
class UserModel 
{
         ...
         public function ExistUnion ($userName, $passWord=null)
	{
		...
		public function ExistUnionjs ($userName)
	{
		$sql = " (SELECT `g_login_id` FROM `j_manage` WHERE g_name = '{$userName}' ) UNION ".
				     " (SELECT `g_login_id` FROM `g_user` WHERE g_name = '{$userName}' ) UNION ".
					 " (SELECT `g_s_name` FROM `g_relation_user` WHERE g_s_name = '{$userName}' ) UNION ".
				     " (SELECT `g_login_id` FROM `g_rank` WHERE g_name = '{$userName}') ";
		return $this->db->query($sql, 0);
	}
	...
	public function Getmanegetj($username)
	{
		$sql = "SELECT * FROM g_login_tj WHERE s_name='{$username}' LIMIT 1";
	...
	public function GetUserModel ($loginId=null, $userName, $passWord=null, $son=false)
	{
		...
		else //查詢子帳號，首選查詢管理員子帳號
		{
			...
			if ($sonUser[0][0] == 89 && Copyright){
				...
				$rFrom = "j_manage";
				//setcookie('rFrom',$rFrom,time()+3600);
	 ...
	 * @param Bool $param
	 */
	public function GetUserMRAll ($name, $param=FALSE)
	{
		if ($param && Copyright)
			$sql = "SELECT g_type, g_panlu_a,g_panlu_b,g_panlu_c, g_danzhu, g_danxiang FROM g_panbiao WHERE `g_nid` = '{$name}' and `g_game_id` = '1' ORDER BY g_id DESC ";
		else 
			$sql = "SELECT g_type, g_a_limit, g_b_limit, g_c_limit, g_d_limit, g_e_limit, g_game_id  FROM g_send_back WHERE `g_name` = '{$name}' ORDER BY g_game_id asc,g_id DESC ";

			return $this->db->query($sql, 1);
	}
	public function GetUserMRid ($name, $gameid=1)
	{
		$sql = "SELECT g_type, g_a_limit, g_b_limit, g_c_limit, g_d_limit, g_e_limit, g_game_id  FROM g_send_back WHERE `g_name` = '{$name}' and `g_game_id` = '$gameid'  ORDER BY g_id DESC ";
		return $this->db->query($sql, 1);
	}
	public function GetUserMRid2 ($name, $gameid=1)
	{
		$sql = "SELECT g_type, g_panlu_a,g_panlu_b,g_panlu_c, g_danzhu, g_danxiang FROM g_panbiao WHERE `g_nid` = '{$name}' and `g_game_id` = '$gameid' ORDER BY g_id DESC ";
		return $this->db->query($sql, 1);
	}
	...
	public function GetUserMR ($name, $param=FALSE)
	{
		if ($param && Copyright)
			...
			$sql = "SELECT g_type, g_a_limit, g_b_limit, g_c_limit, g_d_limit, g_e_limit, g_game_id  FROM g_send_back WHERE `g_name` = '{$name}' and `g_game_id` = '1' ORDER BY g_id DESC ";
	...
	public function GetUserMRK ($name, $param=FALSE)
	{
		if ($param && Copyright)
			$sql = "SELECT g_type, g_panlu_a,g_panlu_b,g_panlu_c, g_danzhu, g_danxiang FROM g_panbiao WHERE `g_nid` = '{$name}' and `g_game_id` = '8'  ORDER BY g_id DESC ";
		else 
			$sql = "SELECT g_type, g_a_limit, g_b_limit, g_c_limit, g_d_limit, g_e_limit  FROM g_send_back WHERE `g_name` = '{$name}' and `g_game_id` = '8' ORDER BY g_id DESC ";
	...
	public function GetUserMRSJAll ($lx)
	{
			$sql = "SELECT g_type, g_a_limit, g_b_limit, g_c_limit, g_d_limit, g_e_limit  FROM  g_send_back_default ORDER BY g_game_id asc,g_id DESC  ";	
			return $this->db->query($sql, 1);
	}
	...
	public function AddUser ($userList)
	{
		...
		`g_distribution`, `g_distribution_limit`, `g_Immediate_lock`, `g_lock`, `g_ip`, `g_date`, `g_uid`,`g_zcgs`,`g_zy`) VALUES(
		'{$userList['g_nid']}',
		'{$userList['g_login_id']}',
		'{$userList['g_name']}',
		'{$userList['g_password']}',
	...
	public function AdddUser ($userListt)
	{
		...
		else if ($userListt['id'] == 2)
		{
			$sql = " SELECT `g_type`, `g_a_limit`, `g_b_limit`, `g_c_limit`, `g_d_limit`, `g_e_limit`, `g_game_id` FROM g_send_back WHERE `g_name` = '{$userListt['L_name']}' ORDER BY g_id ASC ";
		}
		$SendBackList = $this->db->query($sql, 1);
		$sql = " INSERT INTO g_send_back (`g_name`, g_type, `g_a_limit`, `g_b_limit`, `g_c_limit`, `g_d_limit`, `g_e_limit`, `g_game_id`) VALUES ";
		$sql1 = "INSERT INTO g_autolet (`g_nid`, `g_name`, `g_type`, `g_game_id`) VALUES ";
	...
	{
		$sql = "INSERT INTO g_user (g_nid, g_login_id, g_name, g_f_name, g_mumber_type, g_password, g_money, g_money_yes, g_distribution, g_panlus,g_panlu, g_xianer, g_out, g_look, g_ip, g_date, g_uid, money_dx) VALUES (
		'{$userList['g_nid']}',
		'{$userList['g_login_id']}',
		'{$userList['g_name']}',
		'{$userList['g_f_name']}',
	...
	public function AdddMumberUser ($userListt)
	{
		//取出退水盤
		$lenght=0;
		$sql = "SELECT `g_type`, `g_d_limit`, `g_e_limit`, `g_game_id`";
		$P=$userListt['g_panlus'];
		if(strstr($P,'A')!=''){$sql.=',g_a_limit ';$lenght++;}
		if(strstr($P,'B')!=''){$sql.=',g_b_limit ';$lenght++;}
		if(strstr($P,'C')!=''){$sql.=',g_c_limit ';$lenght++;}
		$sql = $sql."FROM `g_send_back` WHERE `g_name` = '{$userListt['s_L_Name']}' ORDER BY g_id ASC ";
		...
		for ($i=0; $i<count($result); $i++)
		{
			$sql.= "('{$userListt['g_name']}', '{$result[$i][0]}', '{$result[$i][1]}', '{$result[$i][2]}', '{$result[$i][3]}'"; 			         for($m=1;$m<=$lenght;$m++){
			$flag=3+$m;
		//	$sql.=",'{$result[$i][$flag]}'";
		
		if($userListt['g_tuishui']+$result[$i][$flag] > 100 ) {
		$sql.=",'100'";
		}
		if($userListt['g_tuishui']+$result[$i][$flag] <= 100 ) {
			$temp=$result[$i][$flag]+$userListt['g_tuishui'];
			$sql.=",'{$temp}'";
		}
			}
