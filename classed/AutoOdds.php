...
class AutomaticOdds 
{
	...
	public function UpExecution()
	{
		...
		if ($result['Num'] && Copyright)
		{
			...
			{
				$sql = "UPDATE g_odds SET `{$key}`={$key}-{$value} WHERE g_type <> 'Ball_9' AND g_type <> 'Ball_10'";
				$this->db->query($sql, 2);
			}
		}
		if ($result['Str'] && Copyright)
		{
			...
			{
				$sql = "UPDATE g_odds SET `{$result['Str'][$i][1]}` = {$result['Str'][$i][1]}-{$result['Str'][$i][2]} WHERE g_type ='{$result['Str'][$i][0]}' LIMIT 1 ";
				$this->db->query($sql, 2);
			}
		}
	}
	...
	{
		$ResultNumber = history_result(0);
		
		//得到雙面無出期數
		global $BallString, $BallString_a;
		$NumberStrArr = sum_ball_count_1 ($BallString, $BallString_a, $ResultNumber, 1);
