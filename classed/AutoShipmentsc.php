...
class AutoLet 
{
	...
	private function GetUserCryList()
	{
		...
		{
		//$uui=$this->cryList[$i]['g_type'];
	
			for ($i=0; $i<count($this->cryList); $i++)
			{
				
				if ($this->cryList[$i]['g_type'] == '廣東快樂十分' || $this->cryList[$i]['g_type'] == '幸运农场')
					$str = _getString($this->cryList[$i]['g_mingxi_1'], $this->cryList[$i]['g_mingxi_2']);
				else if($this->cryList[$i]['g_type'] == '北京赛车PK10' || $this->cryList[$i]['g_type'] == '极速赛车')
					$str = _getStringpk($this->cryList[$i]['g_mingxi_1'], $this->cryList[$i]['g_mingxi_2']);
				else if($this->cryList[$i]['g_type'] == '吉林快3')
					$str = _getStringsz($this->cryList[$i]['g_mingxi_1'], $this->cryList[$i]['g_mingxi_2']);
				else if($this->cryList[$i]['g_type'] == '快樂8')
					$str = _getStringkl8($this->cryList[$i]['g_mingxi_1'], $this->cryList[$i]['g_mingxi_2']);
				else
	...
	private function MyCry($userList, $cryList)
	{
		...
		$logIdlen = mb_substr($userList['g_nid'],0, mb_strlen($userList['g_nid'])-32);
		$logId = result_login_id($logIdlen);
		...
		if ($bMoney > 0 && stristr($cryList['g_s_nid'], $userList['g_nid']) && Copyright)
		{
			...
			switch ($logId)
			{
				case 22 : 
					...
					//echo "ok=".$cryList['g_tueishui_4']."|";
					$arr['g_distribution'] = 0;
					$arr['g_distribution_1'] = 0;
					$arr['g_distribution_2'] = 0;
					$arr['g_distribution_4']= $cryList['g_distribution_4'];
