...
/**
	...
	{
		$result = $this->db->query("SELECT * FROM g_kaipan6 WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1  ", 1);
		if ($result)
		{
			switch ($this->cid)
			{
				case '1': $p=17; $g_id = "Ball_11";
						  $oddsList1 = selectOddspk($p, $g_id); //賠率
						  $g_id = "Ball_12";
						  $oddsList2 = selectOddspk($p, $g_id); //賠率
						  $g_id = "Ball_1";
						  $oddsList3 = selectOddspk($p, $g_id); //賠率
						  $g_id = "Ball_2";
						  $oddsList4 = selectOddspk($p, $g_id); //賠率
