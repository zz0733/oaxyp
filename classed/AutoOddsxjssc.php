...
class AutomaticOddsxjssc
{
	...
	public function UpExecution()
	{
		...
		if ($result['Num'] && Copyright)
		{
			...
			{
				$sql = "UPDATE g_odds10 SET `{$key}`={$key}-{$value} WHERE g_type <> 'Ball_6' AND g_type <> 'Ball_7' AND g_type <> 'Ball_8' AND g_type <> 'Ball_9'";
				$this->db->query($sql, 2);
			}
		}
		if ($result['Str'] && Copyright)
		{
			...
			{
				$sql = "UPDATE g_odds10 SET `{$result['Str'][$i][1]}` = {$result['Str'][$i][1]}-{$result['Str'][$i][2]} WHERE g_type ='{$result['Str'][$i][0]}' LIMIT 1 ";
				$this->db->query($sql, 2);
			}
		}
	}
