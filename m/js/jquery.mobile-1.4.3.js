...
function updateFun(num) {
	...
	aTimerNew ? clearTimeout(aTimerNew) : aTimerNew = null;

	if(--num){

		aTimerNew = setTimeout(function () {
		...
		$.ajax({
			...
			success: function (d) {
				...
				if(d.islogin == '0'){
					window.top.location="/m";
				}else{
					updateFun(updateTimeInit);
				}
				if(d.t_credit){
					$("#t_credit").html("\u53ef\u7528\u984d\u5ea6\u003a" + d.t_credit);
				}
				if(d.t_amt){
					$("#t_amt").html("\u4eca\u65e5\u8f93\u8d62\u003a" + d.t_amt);
				}
			},
			error: function () {
				updateFun(updateTimeInit);
