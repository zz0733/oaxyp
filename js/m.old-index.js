...
//					"</div>"];
	...
	eventInit: function () {
	    ...
	    self.element.$menu.find("a").click(function () {
	    	//调用header的跳转方法
	        var headerWin = top.frames['topFrame'];     
            ...
            top.frames["topFrame"].$("#bankLi").removeClass("czBtnOn");
            top.frames["topFrame"].$("#bankLi").addClass("czBtn");
...
window.dealCreditCard = function(action){
	...
	try{
		...
		interval = window.setInterval(function(){
			var frameLeft = top.frames['mainFrame'].frames['left'];
			var frameRight = top.frames['mainFrame'].frames['main'];
			...
			if(frameLeft && frameRight){
				...
				interval2 = window.setInterval(function(){
					try{
	        	        eval("var mainWin = top.frames['mainFrame'].frames['main']; if(navigator.userAgent.toLowerCase().indexOf('safari')!=-1){ mainWin.contentWindow ? (mainWin.contentWindow." + action + ") : mainWin." + action + " ;} else { mainWin." + action + " }");
