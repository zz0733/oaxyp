...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$mxsha1=md5($result[0]['g_type']."@".$result[0]['g_mingxi_1']."@".$arr['s_numcontent']."$".$arr['s_money'].$result[0]['g_distribution_4']);
    $mxmd=$result[0]['g_type']."@".$result[0]['g_mingxi_1']."@".$arr['s_numcontent']."$".$arr['s_money']."$".$arr['s_odds'];
	$sql = "UPDATE g_zhudan SET g_win = null, g_qishu='{$arr['s_number']}', g_mingxi_1='{$arr['s_numtype']}',g_mingxi_2='{$arr['s_numcontent']}', g_jiner = '{$arr['s_money']}' ,mxsha1='{$mxsha1}',mxmd='{$mxmd}'  WHERE g_id ='{$result[0]['g_id']}' LIMIT 1";
	if ($db->query($sql, 2))
	{
		$SumAmount = new SumAmountxyft($arr['s_number'], false, $arr['uid'], $sum);
...
{
	$ps = false;
	$sql = "SELECT g_id FROM g_zhudan WHERE g_t_id = '{$uid}' AND g_type = '极速赛车' AND g_qishu = '{$result[0]['g_qishu']}}' LIMIT 1 ";
	if ($db->query($sql, 0))
		exit(back('很遺憾！'.$uid.'# 注單已有關聯走飛記錄，無法修改。'));
		
	if (!$db->query("SELECT g_qishu FROM g_history4 WHERE g_qishu = '{$result[0]['g_qishu']}' LIMIT 1 ", 0)){
		if ($db->query("SELECT g_id FROM g_kaipan4 WHERE g_qishu = '{$result[0]['g_qishu']}' LIMIT 1", 0))
			$ps = true;
		else
