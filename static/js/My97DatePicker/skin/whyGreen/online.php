...
$total4 = $db->query("SELECT g_name,g_count_time,g_out FROM j_manage where g_out=1 ", 3);
$total=$total1+$total2+$total3+$total4;

$pageNum = 20;
$page = new Page($total, $pageNum);
$result = $db->query("select   *   from   (SELECT g_name,g_count_time,g_out,g_ip,g_mumber_type FROM g_user  where g_out=1   UNION  SELECT g_name,g_count_time,g_out,g_ip,g_login_id FROM g_rank where g_out=1  UNION  SELECT g_name,g_count_time,g_out,g_ip,g_login_id FROM j_manage where g_out=1   UNION  SELECT g_s_name,g_count_time,g_out,1 as g_ip,0 as g_mumber_type FROM g_relation_user where g_out=1 ) as a ORDER BY g_count_time DESC {$page->limit} ", 1);
}else if($lid==1){
$total2 = $db->query("SELECT g_name,g_count_time,g_out FROM g_rank where g_out=1 ", 3);
$total=$total2;
...
$pageNum = 20;
$page = new Page($total, $pageNum);
$result = $db->query("select   *   from   ( SELECT g_name,g_count_time,g_out,g_ip,g_login_id as g_mumber_type FROM j_manage  where g_out=1  ) as a ORDER BY g_count_time DESC {$page->limit} ", 1);
}else{
$total3 = $db->query("SELECT * FROM g_relation_user where g_out=1 ", 3);
$total=$total3;
...
$countma=$db->query("SELECT g_name,g_count_time,g_out,g_ip,g_login_id FROM j_manage  where g_out=1   ",1);
        ...
        <tr>
          ...
          <td background="/Admin/temp/images/tab_05.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><font style="font-weight:bold" color="#344B50">&nbsp;在线人员查看</font></td>
              </tr>
            </table></td>
          ...
          <td class="c"><!-- strat -->
            <table border="0" cellspacing="0" class="conter">
              ...
              <tr style="height:28px" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
                ...
                <td class="left_p6" align="center"><?php echo$result[$i]['g_count_time']?></td>
                <td class="left_p6" align="center"><a  href="javascript:window.location.reload();" title="踢出系統" class="closepo" src="/Admin/temp/images/USER_1.gif" onclick="closeUser('<?php echo$result[$i]['g_name'] ?>',this,'<?php echo$lid ?>')" >踢出系統</a></td>
              </tr>
              <?php }}}?>
            </table>
            <!-- end -->
