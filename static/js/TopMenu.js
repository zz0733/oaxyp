...
function go_web(t_url) {
	window.open(t_url.replace("￥",s_LT),'mainFrame');
}
...
function Limit_URL(url) {
    if (SB_Limit_Time > Today_Second()){
        ...
        parent.frames["mainFrame"].location=url.replace("￥",s_LT);    
    }
}
...
function SelectType(LT) {
    ...
    document.getElementById("bST_3").className="bST_3";
    document.getElementById("bST_4").className="bST_4";
...
function Loading_But(bID) {
    ...
    for (i=0;i<(mBut.length);i++){
        if (mBut[i] instanceof Array) {
		    ...
		    var Color_B = "";
		    if (mBut[i][0]=="賬單" || mBut[i][0]=="備份") Color_B = "  style='color:#bfffc4'";
...
function Save_CacheBet(T) {
    CacheBet[Number(T)]=parent.frames["mainFrame"].document.body.innerHTML;
}
function Load_CacheBet(T) {
    parent.frames["mainFrame"].document.body.innerHTML=CacheBet[Number(T)];
}
