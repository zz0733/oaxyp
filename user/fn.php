...
if ($t=='總和、龍虎'){
	$odds = setodds($n, $odds, $ConfigModel, $user, 1);
} else {
	$odds = setodds($n, $odds, $ConfigModel, $user, 0);
}

$n = trim(strtr($n, "h"," "));
$n = GetBallByString ($t, $n);

$result = GetUserXianEr ($t, $n, $user[0]['g_name']);
$max = GetUser_s ($result, $user,$t,$n);
