...
global $user, $UserOut, $stratGamepk, $endGamexyft;
$gameid=4;
...
if (( strtotime($dateTime) < strtotime($stratGamexyft) &&  strtotime($dateTime) > strtotime($a)) || strtotime($dateTime) > strtotime($endGamexyft)){
	//exit( "( strtotime($dateTime) < strtotime($stratGamexyft) &&  strtotime($dateTime) > strtotime($a)) || strtotime($dateTime) > strtotime($endGamexyft)");
	back('開盤時間為：'.$stratGamexyft.'--'.$endGamexyft);exit;
}
if (!Matchs::isNumber($_GET['v'],1,11)||!Matchs::isString($_GET['n'], 2, 3)||!Matchs::isStringChi($_GET['t'])) 
	exit(back('Error'));
...
$odds = GetOddspk ($t, $n,$gameid); //獲取賠率

$ConfigModel= configModel("`g_max_money`,`g_mix_money`");
if($t == "冠亞和" || $t == "冠、亞軍和"){
$odds = setoddspk($n, $odds,  $user, 1, $ty,$gameid);
}else{
$odds = setoddspk($n, $odds,  $user, 0, $ty,$gameid);
}
$n = trim(strtr($n, "h"," "));
$n = GetBallByStringpk ($t, $n);

$result = GetUserXianErpk ($t, $n, $user[0]['g_name'],$gameid);
$max = GetUser_s_pk ($result, $user,$t,$n,true);
