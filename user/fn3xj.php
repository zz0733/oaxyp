...
if (!Matchs::isNumber($_GET['numberid'],10,10)||!Matchs::isString($_GET['hid'], 3, 4)||!isset($_GET['tid'])) 
	exit(back('Error'));
...
$odds = GetOddscq($s_cq[0][0], $a,$gameid);
...
if ($s_cq[0][0] == 'Ball_1' || $s_cq[0][0] == 'Ball_2' || $s_cq[0][0] == 'Ball_3' || $s_cq[0][0] == 'Ball_4' || $s_cq[0][0] == 'Ball_5')
{
	if ($a != 'h11' &&$a != 'h12' &&$a != 'h13' &&$a != 'h14')
		$odds = setoddscq($a, $odds, $ConfigModel, $user, 0,$s_cq[0][0],$gameid);
	else 
		$odds = setoddscq($a, $odds, $ConfigModel, $user, 1,0,$gameid);
}
else if ($s_cq[0][0] == 'Ball_6')
{
	$odds = setoddscq($a, $odds, $ConfigModel, $user, 1,$s_cq[0][0],$gameid);
}
else if ($s_cq[0][0] == 'Ball_7' || $s_cq[0][0] == 'Ball_8' || $s_cq[0][0] == 'Ball_9')
{
	$odds = setoddscq($a, $odds, $ConfigModel, $user, 2,$s_cq[0][0],$gameid);
}

$c =tcqsm($s_cq[0]);
$result = GetUserXianErcq($c, $user[0]['g_name'],$gameid);
$ccd=isBallType($s_cq[0][0],$s_cq[0][1]);
$max = GetUser_s($result, $user,$ccd[0], $ccd[1],$gameid);
...
<form id="dp" action="inc/DataProcessingjxsscsm.php" method="post" onsubmit = "return submitformscq()">
	<input type="hidden" name="gtypes" value="<?=$gameid?>" />
