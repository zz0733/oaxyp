...
(function(){
	...
	function loadGameInfo(bool){
		var number = $("#number");
		var sy = $("#sy");
		$.post("/AjaxAll/xjsscJson.php", { typeid : 1, mid : _hiden}, function(data){
	...
	function loadDayInfo(){
		$.post("/AjaxAll/xjsscJson.php", { typeid : 2, mid : _hiden}, function(data){
	...
	function loadodds(oddslist, endtime, number){
		...
		for (var n=0; n<oddslist.length; n++){
			for (var i in oddslist[n]){
				odds = oddslist[n][i];
				urls = "fn2xj.php?tid="+bc(a[n])+"&numberid="+number+"&hid="+a[n]+i;
...
if(getCookie("soundbut")=="on" || getCookie("soundbut")==null || getCookie("soundbut")==""){
	...
	function setOpnumberTirem(){
		...
		if (opnumber != ""){
			...
			if (sum == 2 || sum == 882) {
				var interval = setInterval(function(){
					$.post("/AjaxAll/xjsscJson.php", {typeid : 3}, function(data){
