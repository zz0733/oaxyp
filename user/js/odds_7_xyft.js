var _url = "../AjaxAll/oddsJson_7_xyft.php";
...
function loadodds(oddslist, endtime, number){
	...
	for (var n=0; n<oddslist.length; n++){
		for (var i in oddslist[n]){
			odds = oddslist[n][i];
			urls = "fnxyft3.php?v="+number+"&n="+i+"&t=t"+(n+7);
...
//	});
	...
	id.each(function () {
		if ($(this).attr("id") != "") {
			...
			if (endtime >1)
				$(this).html("<input name=\"t"+lock1+"_"+temp+"\" class=\"inp1\"  onclick=\"Shortcut_ImportM(this)\"  onkeyup=\"digitOnly(this)\" onfocus=\"this.className='inp1m'\" onblur=\"this.className='inp1';\" type=\"text\" maxLength=\"9\"/>");
			else
				$(this).html("停押");
		}
		if(count==14) {lock1++;count=0}
	});
}
...
function stringByInt (str){
	switch (str){
		case "第七名" : return setResults[0];
		case "第八名" : return setResults[1];
		case "第九名" : return setResults[2];
		case "第十名" : return setResults[3];
	}
}
