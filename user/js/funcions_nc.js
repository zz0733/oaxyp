...
var URL = "../AjaxAll/Default.ajax_nc.php";
	...
	if (setTime[0] > 0){
		...
		Oclass.each( function () {
			...
			if (_href == "k2"){
				...
				$(this).html("<a id=\""+clas+"\" href=\"fn_nc.php?v="+nunId+"&n="+$thisId+"&t="+tysId+"\" target=\"leftFrame\" class=\"bgh\">"+$odds+"</a>");
		}); 
	}
};
...
setAction[4] = function(lock) { //輸入框
	...
	id.each(function () {
		if ($(this).attr("id") != "") {
			if (lock)
				$(this).html("<input name=\""+$(this).attr("id")+"\" class=\"inp1\"  onclick=\"Shortcut_ImportM(this)\"  onkeyup=\"digitOnly(this)\" onfocus=\"this.className='inp1m'\" onblur=\"this.className='inp1';\" type=\"text\" maxLength=\"9\"/>");
			else
				$(this).html("停押");
		}
	});
...
setAction[6] = function () { //出球率與無出期數
	if (_href.indexOf("g") > -1){
		...
		$.post (URL, {typeid : "sumball", gid : gid}, function (data) {
			//alert(data);return;
			var th1 = '<th><font color="#4A1A04">出&nbsp;球&nbsp;率</font></th>';
			var th2 = '<th><font color="#4A1A04">無出期數</font></th>';
	...
	$.post (URL, {typeid : "sumball_s", href : _href}, function (data) {
		...
		if (data.row_1 != ""){
			for (var key in data.row_1){
				row_1Html.push("<tr bgcolor=\"#fff\" height=\"20\"><td style=\"padding-left:5px; background:#fff4eb; color:#511e02\">"+key+"</td><td style=\"background:#ffffff; width:35px; color:red; text-align:center\">"+data.row_1[key]+" 期</td></tr>");
			}
	...
	var winResult = $("#sy");
		$.post (URL, { typeid : "openNumber"}, function (data) {
			...
			if (NumberCache == null){
				...
				setAction[6]();
				setAction[7]();
				winResult.html((data.winMoney).toFixed(1));
				return;
