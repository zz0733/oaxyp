var _url = "../AjaxAll/oddsJson_nc.php";
...
function loadodds(oddslist, endtime, number){
	...
	for (var n=0; n<oddslist.length; n++){
		for (var i in oddslist[n]){
			odds = oddslist[n][i];
			urls = "fn_nc.php?v="+number+"&n="+i+"&t=t"+(n+1);
...
function loadinput(endtime){
	var loads = $(".loads");
	var count=0, lock1=lock2=lock3=lock4=1, lock5=5, s, n="停押";
	loads.each(function(){
		count++;
		if (count>=9 && count <= 40){
			s = "t"+lock1+"_h2"+lock2;
...
function submitforms(){
	$.post("../AjaxAll/Default.ajax_nc.php", { typeid : "sessionId"}, function(){});
...
function nameformat(array){
	...
	switch (array[1]) {
		case "h1": arr[1] = '總和大'; arr[2]="h1"; break;
		case "h2": arr[1] = '總和單'; arr[2]="h2"; break;
		case "h3": arr[1] = '總和尾大'; arr[2]="h5"; break;
		case "h4": arr[1] = '龍'; arr[2]="h6"; break;
		case "h5": arr[1] = '總和小'; arr[2]="h3"; break;
		case "h6": arr[1] = '總和雙'; arr[2]="h4"; break;
		case "h7": arr[1] = '總和尾小'; arr[2]="h7"; break;
		case "h8": arr[1] = '虎'; arr[2]="h8"; break;
		case "h21": arr[1] = '大'; arr[2]=h+array[1]; break;
		case "h22": arr[1] = '小'; arr[2]=h+array[1]; break;
		case "h23": arr[1] = '單'; arr[2]=h+array[1]; break;
		case "h24": arr[1] = '雙'; arr[2]=h+array[1]; break;
		case "h25": arr[1] = '尾大'; arr[2]=h+array[1]; break;
