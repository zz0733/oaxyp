var _url = "../AjaxAll/oddsJson_sz_xyft.php";
...
function smlen(data) { //兩面長龍
	if (data.num_arr != ""){
		...
		for (var key in data.num_arr){
			row_1Html.push("<tr bgcolor=\"#fff\" height=\"20\"><td style=\"padding-left:5px; background:#fff4eb; color:#511e02\">"+key+"</td><td style=\"background:#ffffff; width:35px; color:red; text-align:center\">"+data.num_arr[key]+" 期</td></tr>");
		}
	...
	for (var n=0; n<oddslist.length; n++){
		for (var i in oddslist[n]){
			...
			//switch(n){
				...
				//case 3 : nn=12;break;
				//}
			urls = "fnxyft3.php?v="+number+"&n="+i+"&t=t"+(n+1);
...
function loadinput(endtime){
	...
	id.each(function () {
		if ($(this).attr("id") != "") {
			...
			if (endtime >1)
				$(this).html("<input name=\""+tt+"_"+temp+"\" class=\"inp1\" onclick=\"Shortcut_ImportM(this)\"  onkeyup=\"digitOnly(this)\" onfocus=\"this.className='inp1m'\" onblur=\"this.className='inp1';\" type=\"text\" maxLength=\"9\"/>");
			else
				$(this).html("停押");
		}
	});
...
function submitforms(){
	$.post("../AjaxAll/Default.ajax_xyft3.php", { typeid : "sessionId"}, function(){});
...
function nameformat(array){
	...
	switch (array[0]){
		...
		case "t11" : h="l"; arr[0] = "冠、亞軍和"; break;
	...
	switch (array[1]) {
		...
		switch (array[1]) {
		case "h1": arr[1] = '3'; arr[2]=h+array[1]; break;
		case "h2": arr[1] = '4'; arr[2]=h+array[1]; break;
		case "h3": arr[1] = '5'; arr[2]=h+array[1]; break;
		case "h4": arr[1] = '6'; arr[2]=h+array[1]; break;
		case "h5": arr[1] = '7'; arr[2]=h+array[1]; break;
		case "h6": arr[1] = '8'; arr[2]=h+array[1]; break;
		case "h7": arr[1] = '9'; arr[2]=h+array[1]; break;
		case "h8": arr[1] = '10'; arr[2]=h+array[1]; break;
		case "h9": arr[1] = '11'; arr[2]=h+array[1]; break;
		case "h10": arr[1] = '12'; arr[2]=h+array[1]; break;
		case "h11": arr[1] = '13'; arr[2]=h+array[1]; break;
		case "h12": arr[1] = '14'; arr[2]=h+array[1]; break;
		case "h13": arr[1] = '15'; arr[2]=h+array[1]; break;
		case "h14": arr[1] = '16'; arr[2]=h+array[1]; break;
		case "h15": arr[1] = '17'; arr[2]=h+array[1]; break;
		case "h16": arr[1] = '18'; arr[2]=h+array[1]; break;
		case "h17": arr[1] = '19'; arr[2]=h+array[1]; break;
	}
