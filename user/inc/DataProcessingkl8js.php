...
global $user, $UserOut, $stratGamekl8, $endGamekl8;
$dateTime = date('Y-m-d H:i:s');
$a = date('Y-m-d ').'09:00:00';
if ( ($dateTime < $stratGamekl8 && $dateTime > $a) || $dateTime > $endGamekl8)
{
	back('開盤時間為：'.$stratGamekl8.'--'.$endGamekl8);exit;
}

$ConfigModel =configModel("`g_web_lock`, `g_kl8_game_lock`");

if ($ConfigModel['g_kl8_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
...
$max = null;
	...
	//dump($result);
	$ccd=isKl8Type($s_cq[0],$s_cq[1]);
	//dump($ccd);
	$max = GetUser_s_kl8($result, $user,$ccd[0], $ccd[1], true);
...
for ($i=0; $i<1; $i++)
{
	...
	$a= $s_cq[0]=="Ball_1"?$s_cq[1]:"h".substr($s_cq[1],-1);// mb_substr($s_cq[1], 1, mb_strlen($s_cq[1]));
	
	$odds = getOddsKl8($s_cq[0], $a);
	
	$odds=setoddskl8($a,$odds,$user,1,$s_cq[0]);
	...
	$DtnArray = SumRankDistribution ($user, $s_cq[2], null, $c, true,true,true);
	//dump($DtnArray);
	...
	if ($ListArr[$i]['id'] == null)
				exit(alert_href("抱歉！{$ListArr[$i]['g_mingxi_2']}下註失敗", '../left.php'));
}
