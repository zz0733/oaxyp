...
if (isset($_SESSION['guid_code']))
{
	...
	if ($action == 'fn')
	{
		...
		$is_Number = isNumbernc($types, $number_2, $number_1,9); 	//驗證提交的期數是否已經封盤
		...
		$h = $_POST['hid'];
		$odds = GetOdds ($types, $h,9); 														//賠率
		if ($types=='總和、龍虎'){
			$odds = setoddsnc($h, $odds, $ConfigModel, $user, 1);
		} else {
			$odds = setoddsnc($h, $odds, $ConfigModel, $user, 0);
		}
	...
	else if ($action == 'fn1') //alert提交
	{
		...
		{
			$is_Number = isNumbernc($s_type, $s_ball_arr[$i], $s_number,9); 
			if ($is_Number != 1)
		...
		if (!Matchs::isNumber($countZhuEr) || $countZhuEr < $ConfigModel['g_mix_money'])
			exit(alert_href('抱歉1！您的下注金額錯誤。', '../left.php'));
		for ($i=0; $i<count($s_ball_arr); $i++)
		{
			...
			$odds = GetOdds ($s_type, $s_hid_arr[$i],9); 														//賠率
			if ($s_type=='總和、龍虎'){
				$odds = setoddsnc($s_hid_arr[$i], $odds, $ConfigModel, $user, 1);
			} else {
				$odds = setoddsnc($s_hid_arr[$i], $odds, $ConfigModel, $user, 0);
			}
	...
	else if ($action == 'fn2') //連碼提交
	{
		...
		for ($i=0; $i<count($s_ball_arr); $i++)
		{
			...
			if ($is_Number != 1) 
			{
			xit(alert_href('抱歉！第 '.$s_number.' 期已經封盤', '../left.php'));
			}
		...
		$odds = GetOdds ('連碼', $hi,9); //賠率
		$odds = setoddsnc($hi, $odds, $ConfigModel, $user, 2);
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumbernc($sm_arr[$i][0], $sm_arr[$i][1], $number_1,9);
			if ($is_Number != 1) exit(alert_href('抱歉！第 '.$number_1.' 期已經封盤', '../left.php'));
			if (!$is_Number) exit("NumberError");
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOdds ($sm_arr[$i][0], $sm_arr[$i][1],9); 														//賠率
			$hv = sresult ($sm_arr[$i][1]);
			if ($sm_arr[$i][0]=='總和、龍虎'){
				$odds = setoddsnc($hv, $odds, $ConfigModel, $user, 1);
			} else {
				$odds = setoddsnc($hv, $odds, $ConfigModel, $user, 0);
			}
			$ListArr[$i]['g_s_nid'] = $user[0]['g_nid'];															//外鍵
			$ListArr[$i]['g_mumber_type'] = $user[0]['g_mumber_type'];					//會員類型
			$ListArr[$i]['g_nid'] = $user[0]['g_name'];															//帳號
