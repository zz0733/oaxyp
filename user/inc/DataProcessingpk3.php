...
if($gtypesid==4){
	$gtypes = '极速赛车';
	global $user, $UserOut, $stratGamexyft, $endGamexyft;
	$ConfigModel= configModel("`g_mix_money`, `g_web_lock`,`g_xyft_game_lock`");
	...
	if (( strtotime($dateTime) < strtotime($stratGamexyft) &&  strtotime($dateTime) > strtotime($a)) || strtotime($dateTime) > strtotime($endGamexyft)){
		back('開盤時間為：'.$stratGamexyft.'--'.$endGamexyft);exit;
	}

	if ($ConfigModel['g_xyft_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
	exit(alert_href('抱歉！盤口未開放。', '../left.php'));
}else{
	$gtypesid=6;
	$gtypes = '北京赛车PK10';
	global $user, $UserOut, $stratGamepk, $endGamepk;
	$ConfigModel= configModel("`g_mix_money`, `g_web_lock`,`g_pk_game_lock`");
	$dateTime = date('Y-m-d H:i:s');
	...
	if ($ConfigModel['g_pk_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
	exit(alert_href('抱歉！盤口未開放。', '../left.php'));
}
//=================================start================================
...
if (isset($_SESSION['guid_code']))
{
	...
	if ($action == 'fn')
	{
		...
		switch($types){
			...
			case '第三名' : $ty="Ball_3";break;
			case '第四名' : $ty="Ball_4";break;
			case '第五名' : $ty="Ball_5";break;
			case '第六名' : $ty="Ball_6";break;
			case '第七名' : $ty="Ball_7";break;
			case '第八名' : $ty="Ball_8";break;
			case '第九名' : $ty="Ball_9";break;
			case '第十名' : $ty="Ball_10";break;
			case '冠、亞軍和' : $ty="Ball_11";break;
			case '冠亞和' : $ty="Ball_12";break;
			}										//下注遊戲玩法
		...
		$result = GetUserXianErpk ($types, $number_2, $user[0]['g_name'],$gtypesid); 	//當前用戶退水列表
		$max = GetUser_s_pk ($result, $user,$types,$number_2,true);													//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		$gMoney = $max['KeYongEr'] - $money; 											//可用金額
		$is_Number = isNumberpk($types, $number_2, $number_1,$gtypesid); 				//驗證提交的期數是否已經封盤
		if ($is_Number === 2) 
			exit(alert_href('抱歉！第 '.$number_1.' 期已經封盤', '../left.php'));
		if (!$is_Number) 
			exit("NumberError".$types.'----'.$number_2.'--'.$number_1); 																	//驗證號碼是否非法提交
		isUserMoney ($money, $max,$money); 															//驗證下注金額是否大於可用金額
		$countBiShu = 1;																				//總筆數
		$countZhuEr = $money;																	//總注額
		$h = $_POST['hid'];
		$odds = GetOddspk ($types, $h,$gtypesid); 														//賠率
		if ($types=='冠亞和' || $types == "冠、亞軍和"){
			$odds = setoddspk($h, $odds, $user, 1,$ty,$gtypesid);
		} else {
			$odds = setoddspk($h, $odds, $user, 0,$ty,$gtypesid);
		}
		 ...
		 * 重寫個級別退水及占成
		 */
		$DtnArray = SumRankDistributionpk ($user, $money, $number_2, $types,true);
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumberpk($sm_arr[$i][0], $sm_arr[$i][1], $number_1,$gtypesid);
		...
		$max = null;
		for ($i=0; $i<count($sm_arr); $i++){
			$result = GetUserXianErpk ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name'],$gtypesid); 	//當前用戶退水列表
			$max = GetUser_s_pk ($result, $user,$sm_arr[$i][0], $sm_arr[$i][1],true);																			//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOddspk ($sm_arr[$i][0], $sm_arr[$i][1],$gtypesid); 														//賠率
			$hv = sresultpk ($sm_arr[$i][1]);
			...
			if ($sm_arr[$i][0]=='冠亞和' || $sm_arr[$i][0] == "冠、亞軍和"){
			$odds = setoddspk($hv, $odds, $user, 1,$ty,$gtypesid);
			} else {
			$odds = setoddspk($hv, $odds, $user, 0,$ty,$gtypesid);
			}
			...
			$DtnArray = SumRankDistributionpk ($user, $sm_arr[$i][3], $sm_arr[$i][1], $sm_arr[$i][0],true);
			/* 退水修正*/
			$result = GetUserXianErpk ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name'],$gtypesid);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[$i]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
...
new AutoLet($number_1, $ListArr,6);
}
else
{
