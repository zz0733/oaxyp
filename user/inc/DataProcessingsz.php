...
if ( ($dateTime < $stratGamenc && $dateTime > $a) || $dateTime > $endGamenc){
	...
	$ConfigModel= configModel("`g_mix_money`, `g_web_lock`,`g_nc_game_lock`, `g_odds_ratio_b1`,`g_odds_ratio_b2`,`g_odds_ratio_b3`,`g_odds_ratio_b4`,`g_odds_ratio_b5`,`g_odds_ratio_c1`,`g_odds_ratio_c2`,`g_odds_ratio_c3`,`g_odds_ratio_c4`,`g_odds_ratio_c5`");
...
if (isset($_SESSION['guid_code']))
{
	...
	if ($action == 'fn')
	{
		...
		$result = GetUserXianEr ($types, $number_2, $user[0]['g_name'],$gtypesid); 	//當前用戶退水列表
		$max = GetUser_s_ot ($result, $user,$types,$number_2,$gtypesid);													//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		$gMoney = $max['KeYongEr'] - $money; 											//可用金額
		$is_Number = isNumber($types, $number_2, $number_1,$gtypesid); 				//驗證提交的期數是否已經封盤
		if ($is_Number === 2) 
		...
		$h = $_POST['hid'];
		$odds = GetOdds ($types, $h,$gtypesid); 														//賠率
		if ($types=='總和、龍虎'){
			$odds = $setodds_fun($h, $odds, $ConfigModel, $user, 1);
		} else {
			$odds = $setodds_fun($h, $odds, $ConfigModel, $user, 0);
		}
		 ...
		 * 重寫個級別退水及占成
		 */
		$DtnArray = SumRankDistribution ($user, $money, $number_2, $types,$gtypesid);
	...
	else if ($action == 'fn1') //alert提交
	{
		...
		{
			$is_Number = isNumber($s_type, $s_ball_arr[$i], $s_number,$gtypesid); 
			if ($is_Number === 2)
		...
		{
			$result = GetUserXianEr ($s_type, $s_ball_arr[$i], $user[0]['g_name'],$gtypesid); 				//當前用戶退水列表
			$max = GetUser_s_ot ($result, $user,$s_type,$s_ball_arr[$i],$gtypesid);																	//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
			for ($k=0; $k<count($s_money_arr); $k++)
			{
			isUserMoney ($s_money_arr[$k], $max,$countZhuEr); 																//驗證下注金額是否大於可用金額
			}
			$odds = GetOdds ($s_type, $s_hid_arr[$i],$gtypesid); 														//賠率
			if ($s_type=='總和、龍虎'){
				$odds = $setodds_fun($s_hid_arr[$i], $odds, $ConfigModel, $user, 1);
			} else {
				$odds = $setodds_fun($s_hid_arr[$i], $odds, $ConfigModel, $user, 0);
			}
			...
			$DtnArray = SumRankDistribution ($user, $s_money_arr[$i], $s_ball_arr[$i], $s_type,$gtypesid);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
	...
	else if ($action == 'fn2') //連碼提交
	{
		...
		$odds = GetOdds ('連碼', $hi,$gtypesid); //賠率
		$odds = $setodds_fun($hi, $odds, $ConfigModel, $user, 2);
		
		$result = GetUserXianEr ($stringList['type'], null, $user[0]['g_name'],$gtypesid); 				//當前用戶退水列表
		$max = GetUser_s_ot ($result, $user,$stringList['type'],null,$gtypesid);																	//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		$DtnArray = SumRankDistribution ($user, $s_money, null, $stringList['type'],$gtypesid);
		$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[0]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumber($sm_arr[$i][0], $sm_arr[$i][1], $number_1,$gtypesid);
		...
		$max = null;
		for ($i=0; $i<count($sm_arr); $i++){
			$result = GetUserXianEr ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name'],$gtypesid); 	//當前用戶退水列表
			$max = GetUser_s_ot ($result, $user,$sm_arr[$i][0], $sm_arr[$i][1],$gtypesid);																			//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOdds ($sm_arr[$i][0], $sm_arr[$i][1],$gtypesid); 														//賠率
			...
			$DtnArray = SumRankDistribution ($user, $sm_arr[$i][3], $sm_arr[$i][1], $sm_arr[$i][0],$gtypesid);
			/* 退水修正*/
			$result = GetUserXianEr ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name'],$gtypesid);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[$i]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
