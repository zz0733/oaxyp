...
//	dump($c);
	...
	//dump($result);
	$ccd=isKl8Type($s_cq[$i][0],$s_cq[$i][1]);
	//dump($ccd);
...
for ($i=0; $i<count($s_cq); $i++)
{
	...
	$a =$s_cq[0][0]=="Ball_1"?$s_cq[$i][1]:"h".substr($s_cq[$i][1],-1);//mb_substr($s_cq[$i][1], 1, mb_strlen($s_cq[$i][1]));

	$odds = GetOddsKl8($s_cq[$i][0], $a);
...
//	}
	$odds=setoddskl8($a,$odds,$user,1,$s_cq[$i][0]);
	...
	$DtnArray = SumRankDistribution ($user, $s_cq[$i][2], null, $c, true,true,true);
	//dump($DtnArray);
