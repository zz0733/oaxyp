...
if ( strtotime($dateTime) < strtotime($stratGamexyft) || strtotime($dateTime) > strtotime($endGamexyft))
{
...
if (isset($_SESSION['guid_code']))
{
	...
	if ($action == 'fn')
	{
		...
		$result = GetUserXianErpk ($types, $number_2, $user[0]['g_name']); 	//當前用戶退水列表
		$max = GetUser_s_pk ($result, $user,$types,$number_2);													//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		$gMoney = $max['KeYongEr'] - $money; 											//可用金額
		$is_Number = isNumberpk($types, $number_2, $number_1); 				//驗證提交的期數是否已經封盤
		if ($is_Number === 2) 
		...
		$h = $_POST['hid'];
		$odds = GetOddspk ($types, $h); 														//賠率
		if ($types=='冠亞和' || $types == "冠、亞軍和"){
			$odds = setoddspk($h, $odds, $user, 1,$ty);
		} else {
			$odds = setoddspk($h, $odds, $user, 0,$ty);
		}
		 ...
		 * 重寫個級別退水及占成
		 */
		$DtnArray = SumRankDistributionpk ($user, $money, $number_2, $types);
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumberpk($sm_arr[$i][0], $sm_arr[$i][1], $number_1);
		...
		$max = null;
		for ($i=0; $i<count($sm_arr); $i++){
			$result = GetUserXianErpk ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']); 	//當前用戶退水列表
			$max = GetUser_s_pk ($result, $user,$sm_arr[$i][0], $sm_arr[$i][1]);																			//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOddspk ($sm_arr[$i][0], $sm_arr[$i][1]); 														//賠率
			...
			$odds = setoddspk($hv, $odds, $user, 1,$ty);
			} else {
			$odds = setoddspk($hv, $odds, $user, 0,$ty);
			}
			...
			$DtnArray = SumRankDistributionpk ($user, $sm_arr[$i][3], $sm_arr[$i][1], $sm_arr[$i][0]);
			/* 退水修正*/
			$result = GetUserXianErpk ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[$i]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
