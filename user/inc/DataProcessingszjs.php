...
if ( ($dateTime < $stratGamesz && $dateTime > $a) || $dateTime > $endGamesz)
{
	back('開盤時間為：'.$stratGamesz.'--'.$endGamesz);exit;
}

$ConfigModel =configModel("`g_web_lock`, `g_sz_game_lock`");

if ($ConfigModel['g_sz_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
...
$money = $_POST['money'];
if (!Matchs::isNumber($money))
		exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
$s_cq = explode(',', $_POST['s_cq']);
isNumbersz($s_cq[0], $s_number);
$countZhuEr=$money;
$countBiShu=1;
...
$max = null;
	...
	$result = GetUserXianErsz($c, $user[0]['g_name']);
	//dump($result);
	$ccd=isSzType($s_cq[0],$s_cq[1]);
	//dump($ccd);
	$max = GetUser_s_js($result, $user,$ccd[0], $ccd[1], true);
	isUserMoney($money, $max,$countZhuEr);
...
for ($i=0; $i<1; $i++)
{
	...
	$a = mb_substr($s_cq[1], 1, mb_strlen($s_cq[1]));
	
	$odds = GetOddsSz($s_cq[0], $a);
	
	$odds=setoddssz($a,$odds,$user,1,$s_cq[0]);
	...
	$DtnArray = SumRankDistribution ($user, $s_cq[2], null, $c, true,true);
	//dump($DtnArray);
...
new AutoLet($s_number, $ListArr, 7);
                    	...
                    	<td class="t_td_text" colspan="2">
                        	<span class="captions"><?php echo $s_number?>期</span>
                        	 <?php 
	                        	 ...
	                        	 {
									 if($ListArr[$i]['g_mingxi_1']=='三军')
									 {
									 	$nn = $ListArr[$i]['g_mingxi_1'].'『'.(isYxxType($ListArr[$i]['g_mingxi_1'],$ListArr[$i]['g_mingxi_2'])).'』';
									 	echo '<tr><td>&nbsp;註單號：'.$ListArr[$i]['id'].'#</td></tr>
		                        			  <tr><td align="center"><font color="blue">'.$nn.'</font> @ <span style="font-weight:bold; color:red">'.$ListArr[$i]['g_odds'].'</span><br>
											  <font color=blue>開2骰</font> @ <span style="font-weight:bold; color:red">'.(((floatval($ListArr[$i]['g_odds'])-1)*2)+1).'</span><br>
											  <font color=blue>開3骰</font> @ <span style="font-weight:bold; color:red">'.(((floatval($ListArr[$i]['g_odds'])-1)*3)+1).'</span></td></tr>
