<?php
define('Copyright', 'Author QQ: 16213368');
...
if ( !("0000" < $ntime && $ntime < "0827") )	
	$gameList[]=array("id"=>12,'gameid'=>7,'history'=>'g_history7','qs'=>'g_kaipan7');//江苏骰寶
if ( !("0610" < $ntime && $ntime < "0859") )	
	$gameList[]=array("id"=>15,'gameid'=>8,'history'=>'g_history8','qs'=>'g_kaipan8');//北京快樂8
if ( !("0215" < $ntime && $ntime < "0940") )
...
foreach($gameList as $game)
{
	...
	$noTotal=$total_rs?$total_rs[0]['c']:0;
	 ...
	 if($game['id']=="4")
	{
	$url="http://127.0.0.1:303/zhineng/index.php";
	//echo 111;
	} else
	{
	 
     $url="http://www.29x100.cn/api2.php?id=".$game['id'];//采集源1
	}
	...
	switch($game['id'])
	{
		...
		case 4:
			if ($ConfigModel['g_jxssc_op'] == 1 ){
				...
				if ($ConfigModel['g_odds_execution_lock'] == 1 && mb_substr($NowQs, -2) != 1)
				{
					$AutomaticOddsjxssc = new AutomaticOddsxjssc($ConfigModel['g_up_odds_mix_jxssc'], $ConfigModel['g_odds_num_jxssc'], $ConfigModel['g_odds_str_jxssc']);
		...
		case 11:
			if ($ConfigModel['g_pk_op'] == 1 ){
				...
				initializeOddspk();
				//降賠率
				$lastNum = $NowQs - 311446-179*14-179*8;
		...
		if($NowQs!="")
		{
			...
			switch($game['id'])
			{
				...
				if ($lastNum == 180)
					...
					break;
				case 15:
					   $lastNum = $NowQs - 516875-179*14-179*8;
					   if ($lastNum % 179 == 0)
						{
						        $h = intval(date("H"));
						        $day = $h == 0 ? 0 : 1;
	...
	$pkqishu2=$db->query("select count(*) num from g_kaipan8",1);	 
	 if($pkqishu2[0][num]<1)
	   ...
	   // RestoreMoney();
	    ...
	    echo "加載北京快樂8開獎與封盤時間完毕执行时间：". date('Y-m-d H:i:s')." </br>";
	     }	
...
function InsertNumbers_pk ($day=1, $closeTime=2)
{
	...
	$dateArr = array();
	$baseNumber = 311446-179*14-179*8 + (strtotime($insertDate) - strtotime('2012-08-15')) / 86400 * 179;
	$count = 0;
...
{
    global $ConfigModel,$db;
	$sql = " SELECT distinct(g_qishu) FROM  `g_zhudan`  WHERE  `g_type`='北京賽車PK10' and  g_win is null ";
    $result = $db->query($sql, 1);
         if ($result)
		{
			for ($i=0; $i<count($result); $i++)
...
{
    global $ConfigModel,$db;
	$sql = " SELECT distinct(g_qishu) FROM  `g_zhudan`  WHERE  `g_type`='江苏骰寶' and  g_win is null ";
    $result = $db->query($sql, 1);
         if ($result)
		{
			for ($i=0; $i<count($result); $i++)
...
{
    global $ConfigModel,$db;
	$sql = " SELECT distinct(g_qishu) FROM  `g_zhudan`  WHERE  `g_type`='北京快樂8' and  g_win is null ";
    $result = $db->query($sql, 1);
         if ($result)
		{
			for ($i=0; $i<count($result); $i++)
