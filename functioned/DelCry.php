...
if (isset($_GET['delid']) && isset($_GET['sid']))
{
	...
	if ($result)
	{
		...
		$sql=("Insert into g_zhudand select * from g_zhudan  WHERE g_id = '{$delid}' {$or}  ");
        $exe=mysql_query($sql) or  die("<br><font  size=5>删除未成功,请联系管理处理.</font>");
...
else if (isset($_GET['startNumber']) && isset($_GET['endNumber']) && !empty($_GET['startNumber']) && !empty($_GET['endNumber']))
{
	...
	if (isset($_SESSION['GameType']) && $_SESSION['GameType'] == 2)
		...
		$sql=("Insert into g_zhudand select * from g_zhudan  WHERE {$from} and g_qishu >= '{$startNumber}' AND g_qishu <= '{$endNumber}'  ");
        $exe=mysql_query($sql) or  die("<br><font  size=5>删除未成功,请联系管理处理.</font>");
