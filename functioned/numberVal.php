...
function isUserMoney_wap ($money, $max,$totalmoney)
{
	...
	if (!is_numeric($totalmoney))
		exit("MoneyrError");
	if ($money > $max['KeYongEr'])
		exit(back('抱歉！您的可用餘額不足。'));
	if ($money > $max['DanZhu_XianEr'])
		exit(back('抱歉！您的單注限額為 '.$max['DanZhu_XianEr']));
	if (($money+$max['DanQi_YiXia']) > $max['DanQi_XianEr'])
		exit(back('抱歉！您的單期限額為 '.$max['DanQi_XianEr']));
}
 ...
 */
function isNumber ($type=null, $ball=null, $number=null, $gameid)
{
	$bool = false;
	if ($number != null)
	{
		$db = new DB();
		$bool = _isNumberIsNotNull ($db, $ball, $number,$gameid);
	}
	else if ($type != null)
	{
...
function _isNumberIsNotNull ($db, $ball, $number, $gameid)
{
//echo $gameid;
	$bool = false;
	if($gameid==1) $gameid='';
	$result = $db->query("SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan{$gameid}` WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1 ", 1);
...
function isNumbernc ($type=null, $ball=null, $number=null, $gameid=1)
{
     $gameid=1;
	$bool = false;
	if ($number != null)
	{
		$db = new DB();
		$bool = _isNumberIsNotNullnc ($db, $ball, $number,$gameid);
	}
	else if ($type != null)
	{
		if ($type == "第一球" || $type == "第二球" || $type == "第三球" || $type == "第四球" || $type == "第五球" || $type == "第六球" || $type == "第七球" || $type == "第八球" || $type == "總和、龍虎" || $type == "連碼")
...
function _isNumberIsNotNullnc ($db, $ball, $number, $gameid=1)
{
//echo $gameid;
	$bool = false;
	//return 2;
	//echo $bool;
	//if($gameid==9) $gameid='';
	$result = $db->query("SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan9` WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1 ", 1);
	$endTimes = strtotime($result[0]['g_feng_date']);
	$thisTime =  strtotime(date('Y-m-d H:i:s'));
	//echo $result[0]['g_qishu'];
	
	if ($result[0]['g_qishu'] != $number )
	{
		return 2;
		exit();
	}
...
function isNumbercq($type, $number,$gameid=2)
{
	//echo "$type, $number,$gameid";
	if ($type == 'Ball_1' || $type == 'Ball_2' || $type == 'Ball_3' || $type == 'Ball_4' || $type == 'Ball_5' || $type == 'Ball_6' || $type == 'Ball_7' || $type == 'Ball_8' || $type == 'Ball_9')
	{
		$db = new DB();
		$result = $db->query("SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan{$gameid}` WHERE `g_lock` = 2 AND g_qishu = '{$number}' LIMIT 1 ", 1);
		if (!$result || strtotime(date('Y-m-d H:i:s')) > strtotime($result[0]['g_feng_date']))
		{
			//exit(date('Y-m-d H:i:s')." > ".$result[0]['g_feng_date']);
...
function isBallType($type, $ball, $p=false)
{
	...
	if ($type == 'Ball_6')
	{
		switch ($ball)
		{
			...
			case 'h1' : $p==true ? $arr[0] = '總和大小' : $arr[1] = '總和大';break;
			case 'h2' : $p==true ? $arr[0] = '總和大小' : $arr[1] = '總和小';break;
			case 'h3' : $p==true ? $arr[0] = '總和單雙' : $arr[1] = '總和單';break;
			case 'h4' : $p==true ? $arr[0] = '總和單雙' : $arr[1] = '總和雙';break;
			case 'h5' : $p==true ? $arr[0] = '龍虎和' : $arr[1] = '龍';break;
			case 'h6' : $p==true ? $arr[0] = '龍虎和' : $arr[1] = '虎';break;
			case 'h7' : $p==true ? $arr[0] = '龍虎和' : $arr[1] = '和';break;
	...
	else if ($type == 'Ball_7'|| $type == 'Ball_8' || $type == 'Ball_9')
	{
		if ($ball == 'ch1'||$ball == 'dh1'||$ball == 'eh1' ||$ball == 'gh1'||$ball == 'hh1' ||$ball == 'ih1' )
			...
			$arr[1] = '雜六';
		//手机版
        else if ($ball == 'h1'||$ball == 'h1'||$ball == 'h1')
			$arr[1] = '豹子';
		else if ($ball == 'h2'||$ball == 'h2'||$ball == 'h2')
			$arr[1] = '順子';
		else if ($ball == 'h3'||$ball == 'h3'||$ball == 'h3')
			$arr[1] = '對子';
		else if ($ball == 'h4'||$ball == 'h4'||$ball == 'h4')
			$arr[1] = '半順';
		else if ($ball == 'h5'||$ball == 'h5'||$ball == 'h5')
			$arr[1] = '雜六';		
		else 
	...
	else 
	{
		switch ($ball)
		{
			...
			//手机版
			case 'h1' : $arr[1] = 0; break;
			case 'h2' : $arr[1] = 1; break;
			case 'h3' : $arr[1] = 2; break;
			case 'h4' : $arr[1] = 3; break;
			case 'h5' : $arr[1] = 4; break;
			case 'h6' : $arr[1] = 5; break;
			case 'h7' : $arr[1] = 6; break;
			case 'h8' : $arr[1] = 7; break;
			case 'h9' : $arr[1] = 8; break;
			case 'h10' : $arr[1] = 9; break;
			case 'h11' : $arr[1] = '大'; break;
			case 'h12' : $arr[1] = '小'; break;
			case 'h13' : $arr[1] = '單'; break;
			case 'h14' : $arr[1] = '雙'; break;
...
function isNumberpk ($type=null, $ball=null, $number=null, $gameid=6)
{
	//exit($gameid.'ff');
	$bool = false;
	if ($number != null)
	{
		$db = new DB();
		$bool = _isNumberIsNotNullpk ($db, $ball, $number, $gameid);
	}
	else if ($type != null)
	{
...
function _isNumberIsNotNullpk ($db, $ball, $number, $gameid=6)
{
	$bool = false;
	//echo "SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan{$gameid}` WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1 ";exit;
	$result = $db->query("SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan{$gameid}` WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1 ", 1);
	$endTimes = strtotime($result[0]['g_feng_date']);
	$thisTime =  strtotime(date('Y-m-d H:i:s'));
	if ($result[0]['g_qishu'] != $number || $thisTime > $endTimes)
